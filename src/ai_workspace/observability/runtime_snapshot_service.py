...
7개 Analyzer(`ClaudeRuntimeAnalyzer`/`PipelineStageAnalyzer`/
`WorkspaceInfoAnalyzer`/`GitRuntimeAnalyzer`/`GuardianRuntimeAnalyzer`/
`VaultRuntimeAnalyzer`/`McpRuntimeAnalyzer`) 호출만 조합하는 얇은
Service(§13.6 `*Service` 역할 — `RecommendationExplanationService`와
동일한 뼈대). 판정 로직을 전혀 갖지 않는다."""
