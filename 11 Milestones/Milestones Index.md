...
| M51 | Learning Evolution | M49/M50 규칙(실패율 100% + 표본 3건 이상)은 전체 이력만 봐서, 과거에 여러 번 성공했지만 최근 들어 계속 실패하는 추세 악화를 포착하지 못하는 한계를 T01에서 확인. `ExperienceRecord.timestamp`가 이미 정렬 가능해 새 저장소·수집 로직 없이 해결 가능함을 인벤토리. 사용자가 T01에서 "보완(두 규칙 병존)" + "최근 N건 슬라이딩 윈도우"를, T02에서 N=5를 승인 — 승인 시 `recent_failure_streak` 정의 명문화와 Explainability 규칙 구분 기록을 추가 조건으로 요청. `ExperienceStat`에 `recent_failure_streak`(가장 최근 기록부터 거슬러 올라간 연속 실패 횟수) 필드 신설, 최근 5건 연속 실패 시 보류하는 규칙을 기존 규칙에 OR로 추가(대체 아님, 회귀 없음). 어느 규칙이 발동했는지 기존 prose `reason` 채널에 "(M49 규칙)"/"(M51 규칙)"/"(M49+M51 규칙)"로 태깅해 새 필드 없이 Explainability(M44)가 그대로 소비. 새 Domain/Service/Interface 없음 — 기존 파일 2개만 수정. **Milestone Review 완료 — 사용자 승인(2026-08-01)** | ADR-0068 |
| M45-1 | StatusLine Integration Fix | M45 StatusLine이 실제 Claude Code UI에서 표시되지 않는다는 보고를, 추측 대신 공식 문서(`code.claude.com/docs/en/statusline`) 대조로 조사. `.claude/settings.json` 형식과 stdin JSON 필드(`model.display_name`/`effort.level`/`context_window.*`)는 공식 문서와 일치함을 확인 — 실제 원인은 `statusline_main.py`의 `ai_workspace.*` import가 `try/except` 바깥에 있어 import 실패 시 아무 출력 없이 프로세스가 죽는 것(공식 Troubleshooting "Status line not appearing"과 일치). import를 `main()` 내부로 이동해 모든 예외가 항상 한 줄 출력으로 대체되도록 수정, 실패 시에만 남기는 디버그 로그(`/tmp/statusline.log`, `AI_WORKSPACE_STATUSLINE_DEBUG=1`로 opt-in 캡처) 추가. 새 Domain/Interface/Service 없음 — 기존 파일 1개만 수정. Workspace Trust 미승인 등 사용자 환경 문제는 코드로 고칠 수 없어 DoD에 사용자 확인 항목으로 남김(헤드리스 원격 세션이라 실제 UI 접근 불가). **PR #50 Merge 완료(2026-08-01) — 실제 UI 표시는 사용자 확인 대기** | ADR-0069 |
