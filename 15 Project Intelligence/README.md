...
## `Recommendation Execution.md` (Milestone 36/37, ADR-0050/ADR-0051)
...
`EngineSelectionPolicy`(M17)로 실제 실행하고, 그 결과에 따라 기존
Task 상태 전이 기계(M28)로 상태까지 갱신한 기록이다(`vault.
recommendation_execution.write_recommendation_execution_report()`).
**이 문서만 유일하게 "보고"가 아니라 "실행"의 결과를 담는다** —
나머지 문서는 전부 Read Only 리포트다.
...
- **Task Status 이력**(Milestone 37, ADR-0051): 실행 시작 시
  `todo→in-progress`, 성공 시 `in-progress→review`, 실패 시
...
- `.ai/TASKS.md`의 "Milestone 29 — Project Intelligence"/"Milestone
  ...
  Execution"/"Milestone 37 — Task Lifecycle" 절
- `.ai/DECISIONS.md`의 ADR-0043/ADR-0044/ADR-0045/ADR-0046/ADR-0047/
  ADR-0048/ADR-0049/ADR-0050/ADR-0051
