...
#### P1-3: Task 도메인 모델 및 상태 전이 규칙 구현
...
#### P1-6: Project/Task 생성·조회 최소 CLI 명령 구현
- 목적: 사람이 실제로 Project/Task를 다뤄볼 수 있는 최소 진입점을 제공한다.
- 작업 내용: `cli/`에 Project/Task 생성·조회 명령을 구현한다.
- 완료 조건(DoD): CLI로 Project 생성 → Task 생성 → 조회가 end-to-end로 동작한다.
- 상태: TODO
- 의존성: P1-2, P1-3, P1-5

#### P1-7: 단위 테스트 작성
- 목적: Test Before Complete 원칙에 따라 Phase 1 산출물의 정확성을 검증한다.
- 작업 내용: `tests/domain/`, `tests/storage/`, `tests/cli/`에 각 컴포넌트별
  테스트를 작성한다.
...
#### P1-9: ADR-0004 상태 갱신
- 목적: 파일 기반 저장 결정을 정식으로 확정한다.
- 작업 내용: `.ai/DECISIONS.md`의 ADR-0004 상태를 "제안"에서 "승인됨"으로
  갱신한다.
- 완료 조건(DoD): ADR-0004 상태가 "승인됨"으로 표시된다.
- 상태: TODO
- 의존성: P1-5

#### P1-10: Phase 1 완료 승인 요청
- 목적: Approval Required 원칙에 따라 Phase 1 산출물을 검토받는다.
- 작업 내용: 도메인 모델, 저장소, CLI, 테스트 결과를 제시하고 승인을 요청한다.
