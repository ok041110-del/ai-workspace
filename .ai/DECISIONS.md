...
## ADR-0092: Provider Concurrency Management — 엔진별 동시 실행 상한 (Milestone 74)

- 배경: 사용자가 "여러 Agent가 동시에 동일 Provider(Claude/Codex/Gemini 등)를 선택할 때 Provider의 동시 실행 한계를 안전하게 관리"하는 M74를 요청했다. M64~M70이 쌓아온 선택 로직(비용/신뢰도/실행 이력/합의)은 전부 "어떤 엔진이 더 나은가"만 판단했고, "그 엔진이 지금 몇 개나 동시에 실행 중인가"는 전혀 보지 않았다 — 실제 Provider의 API rate limit을 초과할 위험이 코드로 확인됐다.
- 사용자 승인(AskUserQuestion, 2회):
  1. 적용 범위 — **`InMemoryEngineRuntime`과 `ManagedEngineRuntime` 둘 다 구현(권장안 채택)**: M65~M70이 두 구현체를 항상 함께 확장해 온 기존 패턴과 일치하며, 실제로 여러 Agent(스레드)가 동일 인스턴스의 `run()`을 동시 호출할 수 있는 시나리오 모두에 적용된다.
  2. 설정 API — **`register_engine()`에 선택적 keyword 추가(권장안 채택)**: `register_engine(name, adapter, *, max_concurrency: int | None = None)`. `EngineRuntime` 추상 메서드 시그니처는 이 하나만 확장(M14가 `run()`에 `model`을 추가한 것과 동일한 패턴)하고, 기존 호출부는 전혀 수정할 필요가 없다(기본값 `None`=무제한, 100% 하위 호환). `RecoveringEngineRuntime.register_engine()`/`FakeEngineRuntime.register_engine()`도 이 키워드를 그대로 전달(또는 수용)하도록 갱신했다.
- 결정:
  1. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에 `_max_concurrency: dict[str, int]`(register_engine에서 채움)와 `_in_flight: dict[str, int]`(실제 동시 실행 수, `threading.Lock`으로 원자적 증감)를 신설.
  2. `_has_capacity(name)`(읽기 전용 스냅샷 확인) 필터를 `_select()`/`_build_candidates()`(`ManagedEngineRuntime`은 `_require_adapter()`/`_build_candidates()`)의 신뢰도 필터와 같은 자리에 추가 — 한도에 도달한 엔진은 후보에서 제외되어, capacity가 있으면 기존과 동일하게 선택되고 없으면 다른 후보로 자동 fallback한다. 후보가 전혀 남지 않으면 새 예외를 만들지 않고 기존 `NoSuitableEngineError`를 그대로 던진다("모든 Provider가 busy이면 기존 예외 처리 정책을 그대로 따른다"는 요구사항 그대로).
  3. `_try_acquire(name)`(Lock 보호 원자적 check-and-increment)/`_release(name)`(Lock 보호 감소)을 신설하고, `_select_and_acquire()`(`InMemoryEngineRuntime`)/`_require_adapter_and_acquire()`(`ManagedEngineRuntime`)가 "후보 필터링(스냅샷) → 실제 획득" 사이의 경쟁을 처리한다 — 획득에 실패하면(다른 호출이 그 사이 슬롯을 먼저 차지함) 그 엔진을 제외하고 다시 선택해 남은 후보로 안전하게 fallback한다.
  4. `run()`/`run_parallel()`이 세션을 만들기 전 슬롯을 획득하고, `finally` 블록에서 반드시 해제한다(정상 완료/실패/예외/`ManagedEngineRuntime`의 타임아웃 경로 모두 포함) — "동시 실행 카운트는 EngineRuntime 내부 in-process 상태로만 관리"라는 요구사항대로 영속화하지 않는다.
  5. `run_ensemble()`/`run_ensemble_auto()`(M62/M68)는 이 범위에서 제외했다 — `run_ensemble()`은 호출자가 여러 Provider를 정확히 지정해 의도적으로 동시에 비교하는 기능이고, `run_ensemble_auto()`도 "top_n개를 동시에 비교"가 목적이라 개별 호출을 순차적으로 라우팅하는 `run()`/`run_parallel()`과 근본적으로 다른 계약이다. `EngineRuntime` 계약(추상 메서드 시그니처)은 `register_engine()` 하나만 확장했고 새 Core Domain Interface는 만들지 않았다(30종 유지).
- 대안:
  1. `register_engine()`은 그대로 두고 별도 `set_max_concurrency(name, limit)` 메서드를 구현체별로 추가 — 기각(사용자가 `register_engine()` 확장을 선택): `RecoveringEngineRuntime`이 이 메서드를 모르므로 감싸는 Runtime에서는 설정이 불가능해지고, 설정 시점이 "등록"과 분리되어 "등록 직후부터 한도가 적용된다"는 불변식을 코드로 보장하기 어렵다.
  2. `ManagedEngineRuntime`에만 구현(실제 스레드 기반 동시성이 있는 곳) — 기각(사용자가 둘 다 구현을 선택): `InMemoryEngineRuntime`도 여러 Agent(외부 스레드)가 같은 인스턴스의 `run()`을 동시 호출할 수 있고, M65~M70이 항상 두 구현체를 함께 확장해 온 기존 일관성 원칙과도 맞지 않는다.
  3. `run_ensemble()`/`run_ensemble_auto()`에도 동일한 capacity 필터·카운팅 적용 — 기각(YAGNI): 두 메서드는 "여러 Provider를 의도적으로 동시에 비교"가 존재 이유라, 그중 하나를 capacity 이유로 건너뛰면 비교 결과 자체가 왜곡되고 M62(ADR-0080)의 원래 설계 의도와 충돌한다.
- 이유: `register_engine()`에 선택적 keyword를 추가하는 방식은 M14의 `model` 파라미터 확장과 동일한, 이 프로젝트에서 이미 검증된 "계약은 그대로, 선택적 확장만 추가" 패턴이다. capacity 필터를 기존 신뢰도/실행 이력 필터와 같은 `_build_candidates()`/`_select()` 파이프라인 안에 배치해, 완전히 새로운 라우팅 경로를 만들지 않고 기존 선택 로직에 자연스럽게 합류시켰다. Lock 기반 원자적 획득 + 실패 시 제외-재선택 루프는 "in-process 상태로만 관리"라는 단순함 요구와 "실제로 한도를 넘기지 않는다"는 정확성 요구를 모두 만족하는 최소 구조다.
- 결과/영향: `interfaces/engine_runtime.py`(`register_engine()` 시그니처 확장, docstring 갱신, 계약 그 외 무변경), `runtime/engine/engine_runtime.py`/`runtime/engine/managed_engine_runtime.py`(`_max_concurrency`/`_in_flight`/`_concurrency_lock` 신설, `_has_capacity()`/`_try_acquire()`/`_release()`/`_select_and_acquire()`(`_require_adapter_and_acquire()`) 추가, `run()`/`run_parallel()`에 획득/해제 배선), `runtime/engine/recovering_engine_runtime.py`(`register_engine()` 위임 시그니처 갱신), `tests/interfaces/fakes.py`(`FakeEngineRuntime.register_engine()` 시그니처만 수용) 수정. 새 Core Domain Interface 없음(기존 `EngineRuntime`의 메서드 시그니처 1개만 확장, 30종 유지). `tests/runtime/engine/test_engine_runtime.py`에 재진입(reentrant) Adapter 기법으로 결정적 동시성을 재현하는 신규 테스트 6건(capacity 있으면 기존과 동일 선택/capacity 도달 시 자동 fallback/전부 busy면 기존 예외/해제 후 재사용 가능/비용 기반 선택 경로에서도 필터 적용 등), `tests/runtime/engine/test_managed_engine_runtime.py`에 `ThreadPoolExecutor` 기반 실제 동시성 신규 테스트 3건(fallback/여유 있을 때 회귀 없음/전부 busy 시 개별 Task 실패 격리) 추가, 5회 연속 실행으로 타이밍 안정성 확인. `pytest` 1331개(신규 8개, 회귀 없음)/`ruff`/`mypy`(233 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Provider Concurrency Management 서술 추가.

## ADR-0093: Diversity Routing — 동률 후보의 Provider 분산 tie-break (Milestone 75)

- 배경: 사용자가 "여러 Agent가 병렬 실행될 때 동일 Provider/Model에 불필요하게 집중되지 않도록 다양성을 고려한 Engine 선택을 지원"하는 M75를 요청했다. 최신 main(M74/ADR-0092까지 반영)을 조사한 결과, M64(비용)/M65(신뢰도)/M69(실행 이력 성공률)/M74(capacity)가 쌓아온 선택 로직은 전부 "어떤 엔진이 더 나은가"와 "지금 그 엔진을 쓸 수 있는가"만 판단했고, 비용·성공률이 완전히 동률인 후보가 여럿일 때 그중 하나(등록 순서상 첫 번째)로만 몰리는 문제(capacity 여유를 낭비)가 코드로 확인됐다.
- 사용자 승인(AskUserQuestion, 2회):
  1. 다양성 신호 — **M74 `_in_flight`(현재 동시 실행 중인 세션 수) 재사용(권장안 채택)**: 새 카운터를 만들지 않고, "지금 이 순간 가장 한가한 Provider"를 그대로 활용한다. "여러 Agent가 병렬 실행될 때"라는 시나리오와 정확히 맞아떨어지고, 요구사항의 "상태는 EngineRuntime 내부 in-process로만 관리"·"기존 구조 재사용"을 모두 만족한다.
  2. 범위(Provider vs Model) — **Provider(engine_name) 수준만(권장안 채택)**: `EngineCandidate`에 `model` 필드가 없고 M14(ADR-0026)가 "Model은 엔진 선택에 관여하지 않는다"고 이미 명시적으로 분리해뒀다. Model까지 다양성 대상에 포함하려면 `_build_candidates()`/`_select()` 시그니처에 `model`을 새로 전달해야 해 기존 계약을 건드리게 되므로 범위를 Provider로 좁혔다.
- 결정:
  1. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에 `_reorder_by_diversity(candidates)`를 신설한다 — `candidate.engine_name`의 현재 `_in_flight` 값(M74 상태, 새 상태 없음) 오름차순으로 안정 정렬(stable sort)한다.
  2. `_build_candidates()`에서 M74 capacity 필터링(`_has_capacity()`)이 끝난 뒤, `_reorder_by_execution_memory()`(M69)를 적용하기 **직전**에 `_reorder_by_diversity()`를 먼저 적용한다. Python `sorted()`의 안정성 덕분에 이후 성공률 정렬이 후보 간 차이를 만들면 다양성 순서는 그 즉시 덮어써지고, 성공률까지 동률일 때만 다양성 순서가 최종 후보 리스트 순서로 남는다 — `EngineSelectionPolicy.select()`의 비용 기준 `min()`도 비용이 다르면 항상 진짜 최저 비용을 고르므로, 다양성은 "비용과 성공률이 모두 완전히 동률"일 때만 개입하는 순수 tie-break가 된다.
  3. `engine_selection_policy`가 주입되지 않은 경로(첫 매칭 루프)는 전혀 건드리지 않는다 — `_build_candidates()`를 거치지 않으므로 다양성이 관여할 여지가 구조적으로 없다(100% 하위 호환).
  4. `run_ensemble()`/`run_ensemble_auto()`(M62/M68)는 M74와 동일한 이유(호출자가 여러 Provider를 의도적으로 동시에 비교/지정하는 기능)로 범위에서 제외했다(YAGNI). `run_ensemble_auto()`의 `_select_top_n()`은 `_build_candidates()`를 그대로 재사용하므로 다양성의 부수 효과를 받지만, 이는 해가 되지 않는 부수적 결과이지 별도로 설계·검증하지 않았다.
  5. `EngineRuntime`(interface)의 public 계약은 전혀 건드리지 않는다 — 새 메서드도, 새 파라미터도 없다. `InMemoryEngineRuntime`/`ManagedEngineRuntime`의 private 메서드(`_reorder_by_diversity()`) 추가와 `_build_candidates()` 내부 배선 한 줄만 수정했다.
- 대안:
  1. 엔진별 누적 선택 횟수를 새로 추적해 라운드로빈 공정성으로 다양성을 판단 — 기각(사용자가 in-flight 재사용을 선택): 새 in-process 상태가 필요하고, "지금 병렬로 몰리는 상황"이 아니라 "장기간 누적 분배"라는 다른 문제를 푸는 것이라 M75의 실제 동기(동시 실행 집중 방지)와 어긋난다.
  2. `EngineCandidate`/`_build_candidates()`/`_select()`에 `model`을 새로 전달해 Provider+Model 다양성까지 포함 — 기각(사용자가 Provider 수준만 선택): M14가 이미 "Model은 엔진 선택에 관여하지 않는다"고 확정해 둔 경계를 다시 여는 것이라 범위가 커지고, 다양성 하나를 위해 그 경계를 허무는 것은 YAGNI에 맞지 않는다.
  3. 다양성 적용 여부를 별도 opt-in 생성자 플래그(`enable_diversity_routing`)로 노출 — 기각: 이 기능은 정의상 "완전한 동률"에서만 발동하는 순수 tie-break라 별도로 끌 필요가 있는 위험한 동작이 아니다(M69의 실행 이력 재정렬도 별도 플래그 없이 `engine_selection_policy` 주입만으로 자동 적용되는 동일한 선례가 있다).
- 이유: `_in_flight` 재사용은 새 상태를 전혀 만들지 않고도(`register_engine()` 시그니처조차 무변경) "여러 Agent가 병렬 실행될 때 Provider 집중을 피한다"는 요구를 정확히 구현한다. `_reorder_by_diversity()`를 `_reorder_by_execution_memory()` 바로 앞에 배치하는 안정 정렬 조합은 이미 M69가 확립한 "정렬 순서로 tie-break 우선순위를 표현"하는 패턴을 그대로 재사용한 것이라, 비용·신뢰도 우선순위를 절대 바꾸지 않는다는 요구를 새 조건문 없이 정렬 안정성만으로 구조적으로 보장한다.
- 결과/영향: `runtime/engine/engine_runtime.py`/`runtime/engine/managed_engine_runtime.py`(`_reorder_by_diversity()` 신설, `_build_candidates()`에 배선 한 줄 추가, 클래스 docstring 갱신) 수정. 새 파일/Core Domain Interface/public 메서드/새 상태 없음(기존 `EngineRuntime`의 계약 완전 무변경, 30종 유지). `tests/runtime/engine/test_engine_runtime.py`에 실제 스레드 기반(`threading.Thread`) 신규 테스트 4건(완전 동률 시 한가한 Provider 우선/비용 우선순위 불변/실행 이력 성공률 우선순위 불변/정책 미주입 시 회귀 없음), `tests/runtime/engine/test_managed_engine_runtime.py`에 `ThreadPoolExecutor` 기반 실제 병렬 신규 테스트 2건(동률 시 분산/비용 우선순위 불변) 추가, 5회 연속 실행으로 타이밍 안정성 확인. `pytest` 1337개(신규 6개, 회귀 없음)/`ruff`/`mypy`(233 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Diversity Routing 서술 추가.

## ADR-0094: Adaptive Load Balancing — 상대 부하율 기반 tie-break 개선 (Milestone 76)

- 배경: 사용자가 "M74(Provider Concurrency)와 M75(Diversity Routing)를 기반으로 현재 Provider의 실시간 부하를 고려하여 Engine을 자동 분산 선택"하는 M76을 요청했다. 최신 main(M75/ADR-0093까지 반영)을 조사한 결과, M75가 이미 "비용·신뢰도·capacity 동률일 때 M74 `_in_flight`(현재 활성 실행 수)가 가장 적은 Provider를 tie-break로 선택"하는 로직을 구현·병합해 M76 요구사항과 거의 동일했다. 다만 M75는 raw `_in_flight` **개수**만 비교했으므로, Provider마다 `max_concurrency` 한도가 다르면(예: 한도 10 중 3개 사용 vs 한도 2 중 1개 사용) 실제 여유를 거꾸로 판단하는 문제가 있었다(테스트로 재현 확인).
- 사용자 승인(AskUserQuestion, 2회):
  1. M76에서 실제로 달라져야 할 지점 — **상대 부하율로 개선(권장안 채택)**: raw `_in_flight` 개수 대신 `in_flight / max_concurrency` 비율로 계산해, Provider마다 concurrency 한도가 다를 때 "더 여유있는" Provider를 정확히 반영한다.
  2. 무제한(`max_concurrency` 미설정) 엔진 처리 — **부하율 0으로 간주(권장안 채택)**: 실제 동시 실행 상한이 없어 병목 위험이 없다는 사실을 그대로 반영해, 무제한 엔진은 항상 유한 엔진보다 먼저 선택되도록 한다.
  3. (구현 중 발견) 무제한 엔진끼리는 부하율이 항상 0.0으로 동률이 되어 M75가 제공하던 "무제한 엔진들끼리 갈리는" 분산이 사라지는 회귀를 테스트로 확인 후 재질의 — **부하율이 동률이면 raw `_in_flight`로 2차 tie-break(권장안 채택)**: `(load_ratio, in_flight)` 튜플로 정렬 키를 확장해, 한도가 다른 엔진 사이에서는 상대 부하율이 우선하고 한도가 같거나 없는 엔진 사이에서는 M75의 기존 raw count 비교로 자연스럽게 fallback한다.
- 결정:
  1. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에 `_load_ratio(name)`을 신설한다 — `max_concurrency`가 설정된 엔진은 `_in_flight.get(name, 0) / max_concurrency`(0.0~1.0), 설정되지 않은 엔진은 `0.0`을 반환하는 read-only 계산이며 새 상태를 만들지 않는다(M74 `_in_flight`/`_max_concurrency`만 읽음).
  2. `_load_rank(candidate)`를 신설해 `(load_ratio, in_flight)` 튜플을 정렬 키로 반환한다 — 부하율이 다르면 부하율이 우선하고, 부하율까지 동률이면 raw `_in_flight` 개수로 2차 비교해 M75의 기존 분산 동작을 100% 보존한다.
  3. `_reorder_by_diversity(candidates)`는 이름·호출 위치(M74 capacity 필터링 이후, `_reorder_by_execution_memory()` 이전)·정렬 안정성을 그대로 유지한 채 정렬 키만 `candidate.engine_name`의 raw in-flight 개수에서 `self._load_rank(candidate)`로 교체한다. M75와 동일하게 비용·성공률 우선순위는 절대 바뀌지 않으며, `engine_selection_policy` 미주입 시(첫 매칭 경로)와 `run_ensemble()`/`run_ensemble_auto()`도 M75와 동일하게 범위 밖이다.
  4. `EngineRuntime`(interface)의 public 계약은 전혀 건드리지 않는다 — 새 메서드도, 새 파라미터도 없다. `InMemoryEngineRuntime`/`ManagedEngineRuntime`의 private 메서드(`_load_ratio()`, `_load_rank()`) 추가와 `_reorder_by_diversity()` 내부 정렬 키 교체만 수정했다.
- 대안:
  1. raw `_in_flight` 개수를 그대로 유지(M75 동작 재문서화만) — 기각(1차 질의에서 사용자가 상대 부하율 개선을 선택): Provider마다 `max_concurrency`가 다른 실제 운영 시나리오에서 여유를 거꾸로 판단하는 문제를 방치하게 된다.
  2. 무제한 엔진을 부하율 무한대(항상 나중 순위)로 취급 — 기각(사용자가 부하율 0 선택): 무제한 엔진은 병목 위험이 구조적으로 없으므로 항상 먼저 선택하는 것이 "여유"의 실제 의미에 부합한다.
  3. 부하율이 동률일 때 추가 tie-break 없이 등록 순서(원래 리스트 순서)에 맡김 — 기각(사용자가 raw in_flight 2차 tie-break 선택): 무제한 엔진 조합에서 M75가 이미 검증한 "완전 동률 시 분산" 테스트 2건이 회귀하는 것을 확인했고, 이를 방치하면 M76이 M75의 기존 보장을 깨뜨리는 셈이 된다.
- 이유: `_load_ratio()`/`_load_rank()` 모두 M74가 이미 관리하는 `_in_flight`/`_max_concurrency`만 읽는 read-only 계산이라 "상태는 EngineRuntime 내부 in-process로만 관리"·"기존 구조 최대한 재사용" 요구를 새 상태 없이 만족한다. `_reorder_by_diversity()`의 호출 위치·정렬 안정성을 그대로 유지했으므로 M75가 이미 구조적으로 보장한 "비용·신뢰도가 이 tie-break를 항상 덮어쓴다"는 성질이 그대로 이어진다 — 정렬 키만 교체했을 뿐 tie-break가 개입하는 조건(완전 동률) 자체는 전혀 넓어지지 않았다.
- 결과/영향: `runtime/engine/engine_runtime.py`/`runtime/engine/managed_engine_runtime.py`(`_load_ratio()`/`_load_rank()` 신설, `_reorder_by_diversity()` 정렬 키 교체, 클래스 docstring 갱신) 수정. 새 파일/Core Domain Interface/public 메서드/새 상태 없음(기존 `EngineRuntime`의 계약 완전 무변경, 30종 유지). `tests/runtime/engine/test_engine_runtime.py`/`tests/runtime/engine/test_managed_engine_runtime.py`에 상대 부하율이 raw in-flight 개수보다 우선함을 검증하는 신규 테스트 각 1건(서로 다른 `max_concurrency`를 가진 두 엔진에 서로 다른 개수의 동시 실행을 재현) 추가, M75의 기존 6개 다양성 테스트가 회귀 없이 그대로 통과함을 확인, 5회 연속 실행으로 타이밍 안정성 확인. `pytest` 1339개(신규 2개, 회귀 없음)/`ruff`/`mypy`(233 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Adaptive Load Balancing 서술 추가.

## ADR-0095: Engine Benchmark & Capability Profiling — Provider별 성능 프로파일 조회 (Milestone 77)

- 배경: 사용자가 "M65~M76에서 축적된 실행 결과를 바탕으로 Provider/Model별 객관적인 성능 프로파일을 생성하여 이후 Routing의 근거 데이터로 활용"하는 M77을 요청했다. 최신 main(M76/ADR-0094까지 반영) 기준으로 조사한 결과, M65(`EngineReliabilityStat`, engine_name 단독 키, 모든 실행 경로 반영)와 M69(`EngineExecutionMemoryStat`, `(required_capabilities, engine_name)` 키, latency 포함이지만 `run()`/`run_ensemble_auto()` 경로만 기록)가 이미 필요한 원시 데이터를 in-process로 누적하고 있었지만, 이를 하나의 "Benchmark Profile"로 합쳐 조회할 수 있는 public API는 없었다.
- 사용자 승인(AskUserQuestion, 2회):
  1. Model 차원 — **Provider(engine_name) 수준만(권장안 채택)**: 어떤 M65~M76 통계도 model을 키로 쓰지 않고(`EngineCandidate`에도 model 필드가 없음), M14(ADR-0026)가 "model은 엔진 선택에 관여하지 않는다"를 이미 확정해 뒀으며 M75도 같은 이유로 Provider 수준으로 범위를 좁힌 선례가 있다. model까지 추적하려면 `run()`/`_record_*()` 호출부에 새 기록 지점을 추가해야 해 "새로운 측정 시스템은 만들지 않는다"는 요구와 충돌한다.
  2. Latency 집계 범위 — **Execution Count/Success/Failure는 `EngineReliabilityStat`(모든 실행 경로), Latency는 `EngineExecutionMemoryStat` 집계(기록된 경로만, 표본 부족 시 None)(권장안 채택)**: `_record_execution_memory()`(latency 기록)는 `run()`/`run_ensemble_auto()`에서만 호출되고 `run_parallel()`/`run_ensemble()`은 `_record_engine_outcome()`(count/success/failure만)만 호출한다는 점을 코드로 확인 후, 두 기존 통계를 각각의 강점대로 그대로 읽어 결합하기로 했다.
- 결정:
  1. `src/ai_workspace/domain/engine_benchmark.py`에 `EngineBenchmarkProfile`(frozen dataclass)을 신설한다 — `engine_name`, `execution_count`/`success_count`/`failure_count`(M65 `EngineReliabilityStat` 그대로), `latency_sample_count`/`total_latency_seconds`(M69 `EngineExecutionMemoryStat`를 `engine_name`으로 필터링해 합산)를 보관하고, `success_rate()`/`failure_rate()`/`average_latency_seconds()`를 계산해 반환한다(표본 0이면 `None`).
  2. `EngineRuntime`(interface)에 `benchmark_profile(engine_name: str) -> EngineBenchmarkProfile` 추상 메서드를 신설한다(M70의 `consensus_weight()`와 동일한 방식의 계약 확장 — 새 Core Domain Interface가 아니라 기존 인터페이스에 read-only 조회 메서드 하나를 추가한 것). 기록이 전혀 없어도 예외 없이 카운트 0인 프로필을 반환한다(`consensus_weight()`의 "미기록도 값 반환" 원칙과 동일).
  3. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에서 `_engine_reliability.get(engine_name)`과 `_execution_memory`를 `engine_name`으로 필터링해 합산하는 순수 read-only 계산으로 구현한다 — 새 상태(dict/카운터)를 추가하지 않는다.
  4. `RecoveringEngineRuntime`(데코레이터)은 내부 Runtime에 그대로 위임하고, 테스트 대역 `FakeEngineRuntime`/`RecordingEngineRuntime`/`SpyEngineRuntime`/`ScriptedEngineRuntime`에도 계약을 만족하는 최소 구현(빈 프로필 반환 또는 `NotImplementedError`)을 추가한다.
  5. Routing 로직(`_select()`/`_build_candidates()`/`_reorder_by_diversity()`/`_reorder_by_execution_memory()`)은 단 한 줄도 수정하지 않는다 — `benchmark_profile()`은 이 파이프라인 어디에서도 호출되지 않는 순수 조회 전용 API다.
- 대안:
  1. Provider+Model 조합까지 추적하도록 `run()`에 `(engine_name, model)` 키의 새 상태 dict를 추가 — 기각(사용자가 Provider 수준만 선택): "새로운 측정 시스템은 만들지 않는다"는 요구와 M14/ADR-0026의 경계를 다시 여는 문제가 겹친다.
  2. `EngineExecutionMemoryStat`만으로 Execution Count/Success/Failure/Latency를 모두 집계(단일 소스 일관성 우선) — 기각(사용자가 이원화된 소스 결합 선택): `run_parallel()`/`run_ensemble()`으로만 실행된 엔진은 이 통계에 전혀 기록되지 않아 Execution Count가 실제보다 과소 집계된다.
  3. Benchmark Profile을 별도 새 Core Domain Interface(`BenchmarkRepository` 등)로 분리 — 기각: "새로운 Core Domain Interface는 추가하지 않는다"는 요구에 직접 위배되고, M70의 `consensus_weight()` 선례처럼 기존 `EngineRuntime` 계약 확장만으로 충분하다.
- 이유: 두 기존 통계(M65 전체 경로 신뢰도, M69 latency 기록 경로 실행 이력)를 각각의 원래 기록 범위 그대로 읽어 결합하는 것은 "새로운 측정 시스템은 만들지 않는다"·"기존 데이터를 최대한 재사용한다"는 요구를 정확히 만족한다. `EngineBenchmarkProfile`이 `None`을 반환하는 경우(표본 0/latency 미기록)를 명시적으로 표현해, 호출자가 "아직 알 수 없음"과 "실제로 0%/0초"를 혼동하지 않도록 했다 — M69 `EngineExecutionMemoryStat.success_rate()`가 이미 확립한 "표본 부족 시 None" 패턴과 일관된다.
- 결과/영향: `src/ai_workspace/domain/engine_benchmark.py` 신설(`EngineBenchmarkProfile`). `src/ai_workspace/interfaces/engine_runtime.py`(`benchmark_profile()` 추상 메서드 추가), `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`(구현 추가, 클래스 docstring 갱신), `src/ai_workspace/runtime/engine/recovering_engine_runtime.py`(위임 추가) 수정. 테스트 대역 4곳(`tests/interfaces/fakes.py`의 `FakeEngineRuntime`, `tests/agents/test_coding_agent.py`의 `RecordingEngineRuntime`, `tests/core/test_workspace_core.py`의 `SpyEngineRuntime`, `tests/runtime/engine/test_recovering_engine_runtime.py`의 `ScriptedEngineRuntime`)에 계약 준수를 위한 최소 구현 추가. 새 Core Domain Interface 없음(기존 `EngineRuntime`에 read-only 메서드 하나 추가, 30종에서 31종으로 — 계약은 확장되었으나 기존 메서드는 전혀 무변경, 100% 하위 호환). `tests/domain/test_engine_benchmark.py`(도메인 단위 테스트 4건), `tests/runtime/engine/test_engine_runtime.py`(신규 4건: 미기록 시 빈 프로필/reliability+execution memory 집계/여러 capability 조합 합산/Routing 결과 불변 증명), `tests/runtime/engine/test_managed_engine_runtime.py`(신규 2건: latency 기록 경로와 미기록 경로가 섞였을 때 count와 latency 표본 수가 다름을 증명/미실행 엔진 빈 프로필) 추가. `pytest` 1349개(신규 10개, 회귀 없음)/`ruff`/`mypy`(234 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Engine Benchmark & Capability Profiling 서술 추가.

## ADR-0096: Adaptive Engine Benchmark Routing — Benchmark Profile을 Routing tie-break에 반영 (Milestone 78)

- 배경: 사용자가 "M77에서 생성한 Provider/Model Benchmark Profile을 실제 Engine 선택(Routing)에 안전하게 반영"하는 M78을 요청했다. 최신 main(M77/ADR-0095까지 반영) 기준으로 조사한 결과, M77 `benchmark_profile()`은 순수 조회 전용 API로 Routing 파이프라인(`_select()`/`_build_candidates()`/`_reorder_by_diversity()`/`_reorder_by_execution_memory()`) 어디에서도 호출되지 않았다. 이미 M69 `_reorder_by_execution_memory()`가 같은 목적(성공률 기반 tie-break)의 더 좁은 범위(특정 `required_capabilities` 조합) 신호를 우선순위 최상단에서 사용 중이었다.
- 사용자 승인(AskUserQuestion, 3회):
  1. 파이프라인 내 위치 — **`_reorder_by_execution_memory()`와 `_reorder_by_diversity()` 사이(권장안 채택)**: M69가 이미 제공하는, 정확한 capability 조합에서 나온 정밀한 신호가 최우선을 유지하고, M78 Benchmark(Provider 전체 누적, 더 넓은 범위·큰 표본)는 M69가 표본 부족이거나 완전 동률일 때만 개입하는 추가 tie-break로, M75/76 부하 기반 diversity보다는 우선하도록 배치한다.
  2. 지표 결합 방식 — **성공률 우선, 레이턴시로 2차 tie-break(권장안 채택)**: `failure_rate()`는 `success_rate()`와 같은 `execution_count`/`success_count`/`failure_count`에서 나온 보완값이라 별도 정렬 기준으로 쓰지 않는다. 정렬 키는 `(-success_rate, average_latency_seconds)`.
  3. 표본 부족 기준 — **`execution_count < 3`이면 해당 후보는 중립값 처리(권장안 채택)**: M69/M65가 이미 쓰는 "표본 3건 미만은 판정 보류" 규칙을 그대로 재사용한다(새 임계치 발명 안 함, YAGNI). 표본이 부족한 후보는 성공률을 기존 `_NEUTRAL_RATE`(0.5)로, 레이턴시는 `math.inf`로 처리해(비교에서 사실상 제외) 기존 순서(diversity 결과)를 그대로 보존한다 — 페널티도 우대도 주지 않는다.
- 결정:
  1. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에 `_benchmark_rank(candidate) -> tuple[float, float]`을 신설한다 — `self.benchmark_profile(candidate.engine_name)`(M77)을 호출해 `execution_count < _MIN_BENCHMARK_SAMPLES`(3)이면 `(-_NEUTRAL_RATE, math.inf)`를, 아니면 `(-success_rate, average_latency_seconds 또는 math.inf)`를 반환한다.
  2. `_reorder_by_benchmark(candidates)`를 신설해 `_benchmark_rank`로 안정 정렬한다.
  3. `_build_candidates()`에서 `candidates = self._reorder_by_diversity(candidates)` 다음, `return self._reorder_by_execution_memory(...)` 이전에 `candidates = self._reorder_by_benchmark(candidates)`를 삽입한다 — Python `sorted()`의 안정 정렬 특성상 마지막에 적용된 정렬이 최종 우선순위를 결정하므로, 최종 순서는 cost(`EngineSelectionPolicy.select()`) > execution_memory(M69) > benchmark(M78, 신규) > diversity(M75/76)가 된다. 기존 M64/M69 우선순위는 삽입 위치·정렬 순서상 전혀 바뀌지 않는다.
  4. `EngineRuntime`(interface)·`benchmark_profile()` 계약은 전혀 수정하지 않는다 — `_benchmark_rank()`/`_reorder_by_benchmark()`는 두 구현체의 내부 private 메서드일 뿐이며, M77이 이미 노출한 read-only 조회를 내부적으로 재사용할 뿐이다.
  5. `engine_selection_policy`를 주입하지 않으면(첫 매칭 경로) `_build_candidates()` 자체가 호출되지 않으므로 관여하지 않는다. `run_ensemble()`(엔진 이름 직접 지정)은 M74/75/77과 동일한 이유로 범위 밖(YAGNI)이며, `run_ensemble_auto()`의 `_select_top_n()`은 `_build_candidates()`를 그대로 재사용하므로 자동으로 동일한 tie-break를 적용받는다(별도 구현 불필요).
- 대안:
  1. Benchmark tie-break를 `_reorder_by_execution_memory()`보다 먼저(더 높은 우선순위)로 배치 — 기각(사용자가 반대 순서 선택): Provider 전체 누적이 특정 capability 조합의 정밀한 실측 데이터를 덮어쓰면, M69가 원래 의도한 "좁지만 정확한 신호 우선" 원칙이 깨진다.
  2. 표본 부족/latency 부재 시 후보를 후보 목록에서 아예 제외 — 기각: "Benchmark 데이터가 부족하거나 존재하지 않으면 기존 Routing으로 즉시 fallback"이라는 요구는 후보 제외가 아니라 "관여하지 않음"을 의미하며, 제외는 오히려 새로운 필터링 로직(YAGNI 위반)이 된다.
  3. `success_rate`/`failure_rate`/`average_latency_seconds`를 가중합 등으로 하나의 스칼라 점수로 합성 — 기각: 세 지표의 단위·스케일이 달라 임의의 가중치를 새로 발명해야 하고(YAGNI 위반), tuple 기반 사전식 비교가 "성공률이 절대 우선, 레이턴시는 완전 동률일 때만"이라는 요구를 더 명확하고 안전하게 표현한다.
- 이유: M69의 검증된 안정 정렬 파이프라인(`_reorder_by_diversity()` → `_reorder_by_benchmark()` → `_reorder_by_execution_memory()`, 마지막 정렬이 최종 우선순위)에 새 tie-break 단계 하나를 끼워 넣는 것만으로 요구를 충족한다 — 비용·기존 성공률 우선순위 코드를 한 줄도 바꾸지 않고, M77이 이미 읽기 전용으로 조합해 둔 값만 재사용해 새 상태 없이 구현했다. `_NEUTRAL_RATE`/`math.inf` fallback은 M69가 이미 확립한 "표본 부족은 페널티도 우대도 아니다" 원칙을 레이턴시 차원까지 일관되게 확장한 것이다.
- 결과/영향: `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`에 `_MIN_BENCHMARK_SAMPLES` 상수, `_benchmark_rank()`, `_reorder_by_benchmark()` 추가 및 `_build_candidates()`/클래스 docstring 갱신(두 파일 모두 `import math` 추가). `EngineRuntime`(interface)·`benchmark_profile()`·테스트 대역(`FakeEngineRuntime` 등)은 전혀 수정하지 않음 — 계약 완전 무변경, 새 Core Domain Interface·새 public 메서드·새 상태 없음. `tests/runtime/engine/test_engine_runtime.py`(신규 4건: Benchmark가 execution_memory 동률 시 우선/execution_memory 우선순위를 절대 뒤집지 않음/표본 부족 시 diversity로 fallback/정책 미주입 시 미관여), `tests/runtime/engine/test_managed_engine_runtime.py`(동일 시나리오 3건 + `FailableCostedSlowEngineAdapter` 테스트 전용 Adapter 추가) 추가. `pytest` 1356개(신규 7개, 회귀 없음)/`ruff`/`mypy`(234 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Adaptive Engine Benchmark Routing 서술 추가.

## ADR-0097: Adaptive Engine Recommendation — 실행 없는 Engine 추천 API (Milestone 79)

- 배경: 사용자가 "M65~M78에서 축적된 Learning, Benchmark, Routing 정보를 종합하여 현재 Task에 가장 적합한 Provider/Model(또는 Ensemble)을 추천하는 Recommendation Layer"를 요청했다(M79). 최신 main(M78/ADR-0096까지 반영) 기준으로 조사한 결과, M65(신뢰도)/M69(실행 메모리)/M77(Benchmark Profile)/M78(Benchmark tie-break)이 이미 "Task에 가장 적합한 Provider" 판단에 필요한 신호를 전부 갖추고 있었지만, 실행(`run()`)하지 않고 그 판단만 조회할 수 있는 API가 없었다.
- 사용자 승인(AskUserQuestion, 3회):
  1. 추가 위치 — **`EngineRuntime`에 `recommend_engine()` 신규 public 메서드(권장안 채택)**: M70(`consensus_weight()`)/M77(`benchmark_profile()`)과 동일한 방식으로 기존 계약을 확장한다. 별도 `RecommendationEngine` 모듈로 분리하는 대안은 "기존 구조 최대 재사용" 요구와 거리가 있어 기각.
  2. "Workflow Learning" 근거 범위 — **EngineRuntime이 이미 추적 중인 M69 실행 메모리(성공률)를 "학습된 근거"로 그대로 재사용(권장안 채택)**: M71 `WorkflowEngine.recommended_order()`는 Task 순서 학습이지 Engine 선택과는 무관한 별개 계약이며, M70/ADR-0088이 "EngineRuntime은 다른 인터페이스를 호출하거나 알지 못한다"는 결합 금지 원칙을 이미 명시적으로 재확인해 뒀다. `WorkflowEngine`을 선택적 협력자로 주입받는 대안은 결합도를 높이고 workflow_id가 없는 단일 Task 추천에는 적용이 애매해 기각.
  3. 추천 결과 구조 — **단일 `engine_name` + 근거 dict(`evidence`) + 신뢰도 플래그(`confident`), `top_n`으로 여러 개 조회(권장안 채택)**: 단일 스칼라 점수로 합성하는 대안은 지표별 단위가 달라 임의의 가중치를 새로 발명해야 해 기각.
- 결정:
  1. `src/ai_workspace/domain/engine_recommendation.py`에 `EngineRecommendation`(frozen dataclass: `engine_name`/`reason`/`evidence: dict[str, float | None]`/`confident: bool`)을 신설한다.
  2. `EngineRuntime`(interface)에 `recommend_engine(task, required_capabilities=frozenset(), *, top_n=1) -> list[EngineRecommendation]` 추상 메서드를 신설한다(M70/M77과 동일한 방식의 계약 확장 — 새 Core Domain Interface 아님). 후보가 없으면 예외 없이 빈 목록을 반환한다(`run()`과 달리 `NoSuitableEngineError`를 던지지 않음 — "추천은 실행을 강제하지 않는다"는 요구를 그대로 반영).
  3. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에서 `_build_candidates()`/`EngineSelectionPolicy.select()`를 `estimate_cost()`와 동일한 read-only 경로로 반복 호출해(이미 선택된 후보를 제외하고 재선택, `_select_top_n()`과 같은 패턴) 구현한다 — 실제로 `run()`을 호출하지 않으므로 세션 생성/엔진 실행이 전혀 없다. `_build_recommendation()`이 결정(`EngineSelectionDecision`)에 M65(`reliability_success_rate`)/M69(`execution_memory_success_rate`/`latency_seconds`, "Workflow Learning" 근거로 재사용)/M77(`benchmark_success_rate`/`benchmark_average_latency_seconds`)를 채운다. `execution_memory_success_rate`/`benchmark_success_rate`가 모두 표본 부족(3건 미만, M69/M78과 동일 기준)이면 `confident=False`.
  4. `engine_selection_policy`를 주입하지 않으면(첫 매칭 경로) `run()`이 고를 엔진과 같은 엔진을 `confident=False`(근거 데이터 자체가 없음)로 반환한다 — `run()`의 정책 미주입 시 동작과 일관된 원칙.
  5. `RecoveringEngineRuntime`(데코레이터)은 내부 Runtime에 그대로 위임하고, 테스트 대역 4곳(`FakeEngineRuntime`/`RecordingEngineRuntime`/`SpyEngineRuntime`/`ScriptedEngineRuntime`)에 계약을 만족하는 최소 구현을 추가한다.
  6. Routing 로직(`_select()`/`_build_candidates()`/`_reorder_by_diversity()`/`_reorder_by_execution_memory()`/`_reorder_by_benchmark()`)은 단 한 줄도 수정하지 않는다 — `recommend_engine()`은 그 결과를 조회·설명만 하는 순수 API다.
- 대안:
  1. `EngineRuntime` 외부에 독립 `RecommendationEngine`/`RecommendationRepository`를 신설 — 기각(사용자가 `EngineRuntime` 확장 선택): "새로운 Core Domain Interface는 추가하지 않는다"·"기존 구조를 최대한 재사용한다"는 요구에 직접 위배되고, `_build_candidates()`/`EngineSelectionPolicy` 등 내부 상태에 접근하려면 결국 `EngineRuntime`에 새 조회 메서드가 필요해진다.
  2. 생성자에 `workflow_engine: WorkflowEngine | None = None`을 선택적으로 주입받아 `recommended_order()`를 직접 호출 — 기각(사용자가 M69 재사용 선택): `EngineRuntime`이 `WorkflowEngine`이라는 별개 개념을 알게 되어 ADR-0080/0081/M70이 확립한 결합 금지 원칙을 깬다. Task 단위 추천에는 `workflow_id`가 없을 수도 있어 적용 범위도 불명확하다.
  3. `evidence`의 여러 지표를 가중합해 단일 `score: float`로 반환 — 기각(사용자가 dict+confident 플래그 선택): 비용(USD)·성공률(0~1)·latency(초) 단위가 전혀 달라 임의의 가중치 상수를 새로 발명해야 하고(YAGNI 위반), 호출자가 어떤 근거로 추천됐는지 알 수 없게 된다.
- 이유: `_build_candidates()`/`EngineSelectionPolicy.select()`는 M64~M78이 이미 검증한 "Task에 가장 적합한 Provider" 판단 로직 그 자체다. 이를 실행 경로(`run()`)에서 떼어내 read-only로 한 번 더 호출하는 것만으로 "추천"을 구현하면, 별도의 새 판단 로직을 만들지 않고(YAGNI) Routing과 Recommendation이 항상 같은 결론을 낸다는 것을 구조적으로 보장할 수 있다. `confident` 플래그는 M69 `EngineExecutionMemoryStat.success_rate()`/M78 `_MIN_BENCHMARK_SAMPLES`가 이미 확립한 "표본 부족 시 None/중립" 패턴을 "추천 근거 부족 → 기존 Routing 결과 사용" 요구에 맞게 그대로 확장한 것이다.
- 결과/영향: `src/ai_workspace/domain/engine_recommendation.py` 신설(`EngineRecommendation`). `src/ai_workspace/interfaces/engine_runtime.py`(`recommend_engine()` 추상 메서드 추가), `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`(`_build_recommendation()`/`recommend_engine()` 구현 추가, 클래스 docstring 갱신), `src/ai_workspace/runtime/engine/recovering_engine_runtime.py`(위임 추가) 수정. 테스트 대역 4곳(`tests/interfaces/fakes.py`의 `FakeEngineRuntime`, `tests/agents/test_coding_agent.py`의 `RecordingEngineRuntime`, `tests/core/test_workspace_core.py`의 `SpyEngineRuntime`, `tests/runtime/engine/test_recovering_engine_runtime.py`의 `ScriptedEngineRuntime`)에 계약 준수를 위한 최소 구현 추가. 새 Core Domain Interface 없음(기존 `EngineRuntime`에 read-only 메서드 하나 추가 — 계약은 확장되었으나 기존 메서드는 전혀 무변경, 100% 하위 호환). `tests/runtime/engine/test_engine_runtime.py`(신규 5건: 표본 충분 시 confident=True+run() 결과와 동일 엔진/표본 부족 시 confident=False/top_n 비용순 정렬/top_n<1 시 빈 목록/정책 미주입 시 첫 매칭과 동일 엔진+confident=False), `tests/runtime/engine/test_managed_engine_runtime.py`(동일 시나리오 3건) 추가. `pytest` 1364개(신규 8개, 회귀 없음)/`ruff`/`mypy`(235 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Adaptive Engine Recommendation 서술 추가.

## ADR-0098: Autonomous Decision Engine — 추천을 최종 실행 결정으로 통합하는 오케스트레이션 계층 (Milestone 80)

- 배경: 사용자가 "M65~M79에서 구현된 Learning, Benchmark, Workflow Learning, Routing, Recommendation을 하나의 의사결정 계층으로 통합하여, Agent가 실행 직전 최적의 Provider/Model(또는 Ensemble)을 자동 결정하는 Autonomous Decision Engine"을 요청했다(M80). 최신 main(M79/ADR-0097까지 반영) 기준으로 조사한 결과, M79 `recommend_engine()`이 이미 M64(비용)/M65(신뢰도)/M69(실행 메모리)/M77(Benchmark)/M78(tie-break)을 전부 통합한 "1순위 후보"를 계산하고 있었지만, 이를 "추천"이 아니라 "결정"으로 표현하는 API가 없었다.
- 사용자 승인(AskUserQuestion, 3회):
  1. 추가 위치/반환 타입 — **`EngineRuntime.decide_engine()` + 기존 `EngineSelectionDecision`(M17) 재사용(권장안 채택)**: M70/M77/M78/M79와 동일한 방식으로 기존 계약을 확장하고, 새 domain 타입을 만들지 않는다.
  2. 실행 경로 결합 방식 — **기존 `_select()` 무수정, `decide_engine()`은 `recommend_engine()`을 재사용하는 병렬 조회 API(권장안 채택)**: 검증된 `_select()`/`run()` 핵심 경로를 리팩터링하지 않아 회귀 위험이 없다. `decide_engine()`의 engine_name은 `recommend_engine()`의 1순위(= `_build_candidates()`/`EngineSelectionPolicy.select()`와 동일한 파이프라인 결과)를 그대로 쓰므로 수학적으로 `run()`이 실제로 고를 엔진과 항상 같다.
  3. Ensemble 범위 — **단일 결정만(top_n 미지원)(권장안 채택)**: Ensemble 조합 결정은 이미 `run_ensemble_auto()`(M68)가 담당하고 있어, 이번 Milestone은 "실행 직전 최종 1개 결정"에 집중한다.
- 결정:
  1. `EngineRuntime`(interface)에 `decide_engine(task, required_capabilities=frozenset()) -> EngineSelectionDecision` 추상 메서드를 신설한다(M70/M77/M78/M79와 동일한 방식의 계약 확장 — 새 Core Domain Interface 아님). 새 domain 타입을 만들지 않고 M17 `EngineSelectionDecision`(engine_name/model/reason)을 그대로 재사용한다.
  2. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에서 `recommend_engine(task, required_capabilities, top_n=1)`(M79)을 호출해, 결과가 있으면(confident 여부와 무관) 1순위를 `EngineSelectionDecision`에 그대로 담아 반환한다. 결과가 없으면(후보 자체가 없음) `_select()`/`_require_adapter()`를 그대로 호출해 `run()`과 동일한 `NoSuitableEngineError`를 던진다 — "Recommendation 부족 시 기존 EngineSelectionPolicy로 즉시 fallback"을 예외 정책까지 일치시킨다.
  3. **부수 버그 수정**: 구현 중 `recommend_engine()`(M79)의 `engine_selection_policy` 미주입 경로가 `_has_capacity()`(M74)를 확인하지 않아, `_select()`(capacity 필터링함)와 다른(capacity가 꽉 찬) 엔진을 추천할 수 있는 불일치를 발견했다. `decide_engine()`의 "항상 `run()`과 같은 엔진" 보장이 이 불일치에 의존하므로, `recommend_engine()`의 정책 미주입 분기에 `_has_capacity()` 필터를 추가해 `_select()`와 완전히 동일하게 맞췄다(새 알고리즘이 아니라 기존 필터를 빠뜨린 부분을 채운 것).
  4. `RecoveringEngineRuntime`(데코레이터)은 내부 Runtime에 그대로 위임하고, 테스트 대역 4곳(`FakeEngineRuntime`/`RecordingEngineRuntime`/`SpyEngineRuntime`/`ScriptedEngineRuntime`)에 계약을 만족하는 최소 구현을 추가한다.
  5. `run()`/`_select()`/`_build_candidates()`/재정렬 메서드들은 전혀 수정하지 않는다 — `decide_engine()`은 그 결과를 조회·재포장만 하는 병렬 API다.
- 대안:
  1. `_select()`가 내부적으로 `decide_engine()`을 호출하도록 리팩터링해 실행 경로를 하나로 합침 — 기각(사용자가 무수정 유지 선택): 결과는 수학적으로 동일하지만, `run()`/`run_parallel()` 등 모든 실행 경로가 거치는 검증된 핵심 함수를 굳이 리팩터링할 필요가 없다(YAGNI, 회귀 위험 대비 이득 없음).
  2. `EngineRuntime` 외부에 별도 `AutonomousDecisionEngine` 클래스 신설 — 기각: "새로운 Core Domain Interface는 추가하지 않는다"·"상태는 EngineRuntime 내부 in-process로만 관리"라는 요구와 결이 다르고, 결국 `_build_candidates()` 등 내부 상태에 접근하려면 `EngineRuntime`에 새 메서드가 필요해진다(M79와 동일한 논리).
  3. `decide_engine()`에 `top_n`을 지원해 Ensemble 결정까지 포함 — 기각(사용자가 단일 결정만 선택): `run_ensemble_auto()`(M68)가 이미 Ensemble 조합 결정을 담당하므로 중복이며, 이번 Milestone의 요구("실행 직전 최적의 Provider/Model을... 자동 결정")는 단일 결정에 집중된 문구다.
- 이유: `recommend_engine()`(M79)이 이미 `_build_candidates()`/`EngineSelectionPolicy.select()`(M64~M78 전체 통합)의 1순위를 정확히 계산해 두었으므로, 이를 M17의 기존 `EngineSelectionDecision` 포맷으로 재포장하는 것만으로 "여러 신호를 통합한 최종 결정"을 구현할 수 있다 — 새 알고리즘·새 domain 타입·새 실행 경로 없이 "오케스트레이션 계층으로만 동작"하라는 요구를 문자 그대로 만족한다. `run()`과 항상 같은 엔진을 반환한다는 보장은 같은 코드를 재사용하는 구조적 사실에서 나오며, 별도 동기화 로직이 필요 없다.
- 결과/영향: `src/ai_workspace/interfaces/engine_runtime.py`(`decide_engine()` 추상 메서드 추가, `EngineSelectionDecision` import 추가). `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`(`decide_engine()` 구현 추가, `recommend_engine()`의 정책 미주입 분기에 `_has_capacity()` 필터 추가 — 버그 수정, 클래스 docstring 갱신). `src/ai_workspace/runtime/engine/recovering_engine_runtime.py`(위임 추가). 테스트 대역 4곳(`tests/interfaces/fakes.py`의 `FakeEngineRuntime` — `EngineSelectionDecision` import 추가 및 직접 구현, `tests/agents/test_coding_agent.py`의 `RecordingEngineRuntime`, `tests/core/test_workspace_core.py`의 `SpyEngineRuntime`, `tests/runtime/engine/test_recovering_engine_runtime.py`의 `ScriptedEngineRuntime`)에 계약 준수를 위한 최소 구현 추가. 새 Core Domain Interface 없음(기존 `EngineRuntime`에 read-only 메서드 하나 추가, 새 domain 타입 없음, 100% 하위 호환). `tests/runtime/engine/test_engine_runtime.py`(신규 6건: capacity 미확인 버그 수정 검증/정책 주입 시 run()과 동일 엔진/confident 여부에 따른 reason 검증/정책 미주입 시 fallback/후보 없을 때 예외), `tests/runtime/engine/test_managed_engine_runtime.py`(동일 시나리오 5건) 추가. `pytest` 1373개(신규 9개, 회귀 없음)/`ruff`/`mypy`(235 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Autonomous Decision Engine 서술 추가.

## ADR-0099: Workspace Reflection & Continuous Improvement — Decision을 사후 평가하는 Reflection 계층 (Milestone 81)

- 배경: 사용자가 "M80 Autonomous Decision Engine이 내린 실행 결과를 사후 평가(Reflection)하여 다음 실행에서 더 나은 의사결정을 할 수 있도록 Continuous Improvement 계층을 추가"하는 M81을 요청했다. 최신 main(M80/ADR-0098까지 반영, `decide_engine()` 및 그 1순위와 항상 같은 엔진을 고르는 `run()`) 기준으로 조사한 결과, `_build_recommendation()`(M79)이 실행 *전* 근거(evidence)를 계산하는 로직은 있었지만, `run()` 실행이 끝난 뒤 그 근거가 실제 결과와 얼마나 맞았는지 비교·기록하는 계층은 없었다.
- 사용자 승인(AskUserQuestion, 4회):
  1. 적용 범위 — **`run()`만(권장안 채택)**: M80 `decide_engine()`이 "단일 결정만" 범위로 한정한 것과 동일하게, `run_parallel()`/`run_ensemble()`/`run_ensemble_auto()`는 범위 밖(YAGNI). `ManagedEngineRuntime.run_parallel()`은 내부적으로 `run()`을 호출하므로 자동으로 함께 적용된다.
  2. "예상"의 정의 — **evidence 전체 스냅샷(권장안 채택)**: `_build_recommendation()`/`recommend_engine()`(M79)이 이미 만드는 evidence dict(`reliability_success_rate`/`execution_memory_success_rate`/`latency_seconds`/`benchmark_success_rate`/`benchmark_average_latency_seconds`) 구조를 그대로 재사용해, 실행 직전(이번 실행 자체가 통계에 반영되기 전) 스냅샷한다.
  3. 보관 정책 — **엔진별 최근 20개만(권장안 채택)**: 영속화 없음(세션 종료 시 소멸), 엔진별 `deque(maxlen=20)`로 무제한 메모리 증가를 방지한다.
  4. 참고 정보 반영 방식 — **`reason` 텍스트에 문구 추가(권장안 채택)**: `_build_recommendation()`의 `reason`에 최근 회고 불일치 횟수를 문구로만 덧붙인다 — `evidence`/`confident`/순위(Routing 결과)는 전혀 바뀌지 않는다.
- 결정:
  1. `src/ai_workspace/domain/reflection.py`에 `ReflectionReport`(frozen dataclass)를 신설한다 — `engine_name`/`expected_evidence`(dict, M79 evidence와 동일 구조의 스냅샷)/`expected_confident`/`expected_success_rate`(파생값, execution_memory > benchmark > reliability 우선순위로 대표값 선택)/`expected_latency_seconds`(파생값, execution_memory > benchmark 우선순위)/`actual_success`/`actual_latency_seconds`/`expectation_matched`(`expected_success_rate >= 0.5`와 `actual_success`의 일치 여부, 예상 없으면 `None`)/`latency_gap_seconds`(`actual - expected`, 예상 없으면 `None`).
  2. `EngineRuntime`(interface)에 `reflection_reports(engine_name: str | None = None) -> list[ReflectionReport]` 추상 메서드를 신설한다(M70/M77/M79/M80과 동일한 방식의 계약 확장 — 새 Core Domain Interface 아님). read-only 조회이며 기록이 없으면 빈 목록을 반환한다.
  3. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에서 `_build_recommendation()`의 evidence 계산을 `_evidence_snapshot(name, required_capabilities)`(신규 private 헬퍼, M65/M69/M77을 새 측정 없이 읽기 전용 조합 — 기존 `_build_recommendation()` 로직을 그대로 추출)로 공유한다. `run()`이 엔진을 선택하고 실행하기 직전(통계가 이번 실행으로 갱신되기 전) 이 스냅샷을 캡처해, 실행 후 `_record_reflection()`이 실제 결과와 비교해 `ReflectionReport`를 엔진별 `deque(maxlen=20)`에 추가한다. `ManagedEngineRuntime.run()`은 timeout/예외/정상 완료 3개 분기 모두에서 기존 `_record_engine_outcome()`/`_record_execution_memory()` 호출 지점에 나란히 `_record_reflection()`을 추가한다(취소된 결과는 기존과 동일하게 기록하지 않음).
  4. `_build_recommendation()`의 `reason`에 `_reflection_note(name)`(신규 private 헬퍼 — 최근 기록 중 `expectation_matched is False`인 개수를 세어 0건이면 빈 문자열, 아니면 " (최근 회고: 예측-실제 불일치 N회)")을 덧붙인다. `evidence`/`confident`/`recommend_engine()`/`decide_engine()`이 고르는 engine_name(순위)에는 전혀 영향을 주지 않는다 — Reflection은 참고 정보로만 반영되고 즉시 Routing 정책을 바꾸지 않는다는 요구를 그대로 만족한다.
  5. `RecoveringEngineRuntime`(데코레이터)은 내부 Runtime에 그대로 위임하고, 테스트 대역 4곳(`FakeEngineRuntime`/`RecordingEngineRuntime`/`SpyEngineRuntime`/`ScriptedEngineRuntime`)에 계약을 만족하는 최소 구현(빈 목록 반환 또는 `NotImplementedError`, 기존 M79/M80 패턴과 동일)을 추가한다.
  6. `_select()`/`_build_candidates()`/재정렬 메서드/`decide_engine()`은 단 한 줄도 수정하지 않는다 — Reflection은 `run()`이 이미 계산하는 값을 실행 전후로 스냅샷·비교만 하는 관찰 계층이다.
- 대안:
  1. `run()`뿐 아니라 `run_parallel()`/`run_ensemble()`/`run_ensemble_auto()`까지 Reflection 대상에 포함 — 기각(사용자가 `run()`만 선택): M80 `decide_engine()`이 이미 "단일 결정"으로 범위를 좁힌 선례가 있고, `run_ensemble()`류는 여러 엔진을 동시에 비교하는 게 목적이라 "하나의 결정에 대한 예상-실제 비교"라는 Reflection의 전제와 결이 다르다.
  2. "예상"을 성공률/latency 두 스칼라 값으로만 간소화(evidence 전체 스냅샷 대신) — 기각(사용자가 전체 스냅샷 선택): 근거 정보가 줄어들며, `_build_recommendation()`과의 계산 공유(`_evidence_snapshot()`)가 어색해진다.
  3. Reflection 기록을 무제한 보관 — 기각(사용자가 최근 20개 선택): 영속화가 없는 상태에서 장시간 세션이 실행되면 엔진별 리스트가 무한히 자라 메모리 문제가 된다. 다른 M65/M69/M77 통계는 집계값이라 무제한이어도 상수 크기지만, Reflection은 개별 실행 기록이라 다르다.
  4. Reflection 결과를 `evidence` dict에 새 키(`reflection_mismatch_rate` 등)로 추가 — 기각(사용자가 reason 텍스트만 선택): `evidence` dict 키 집합에 의존하는 기존 코드/테스트에 영향을 줄 수 있어 "100% 하위 호환" 요구에 더 안전한 reason 텍스트 추가만 선택했다.
- 이유: `_build_recommendation()`(M79)이 이미 계산해 둔 evidence를 실행 직전 스냅샷으로 재사용하고, `run()`이 이미 계산하는 실제 결과(success/latency)와 비교하는 것만으로 "예상과 실제의 차이"를 구현할 수 있다 — 새 측정 시스템이나 새 Routing 알고리즘 없이 기존 데이터를 읽기 전용으로 한 번 더 조합했을 뿐이다. `expectation_matched`/`latency_gap_seconds`를 구조화된 필드로 남기면서도 Routing에는 `reason` 텍스트로만 영향을 주게 해, "즉시 정책을 바꾸지 않는다"는 요구를 코드 경로 자체로 강제했다(evidence/confident/순위 계산 코드가 Reflection 상태를 전혀 읽지 않는다).
- 결과/영향: `src/ai_workspace/domain/reflection.py` 신설(`ReflectionReport`). `src/ai_workspace/interfaces/engine_runtime.py`(`reflection_reports()` 추상 메서드 추가). `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`(`_evidence_snapshot()`/`_reflection_note()`/`_record_reflection()`/`reflection_reports()` 추가, `_build_recommendation()`이 `_evidence_snapshot()`을 재사용하도록 리팩터링 및 reason에 회고 문구 추가, `run()`에 스냅샷 캡처·`_record_reflection()` 호출 추가, 클래스 docstring 갱신). `src/ai_workspace/runtime/engine/recovering_engine_runtime.py`(위임 추가). 테스트 대역 4곳(`tests/interfaces/fakes.py`의 `FakeEngineRuntime`, `tests/agents/test_coding_agent.py`의 `RecordingEngineRuntime`, `tests/core/test_workspace_core.py`의 `SpyEngineRuntime`, `tests/runtime/engine/test_recovering_engine_runtime.py`의 `ScriptedEngineRuntime`)에 계약 준수를 위한 최소 구현 추가. 새 Core Domain Interface 없음(기존 `EngineRuntime`에 read-only 메서드 하나 추가, 100% 하위 호환). `tests/runtime/engine/test_engine_runtime.py`(신규 6건: 예상 없을 때 None 판정/예상-실제 불일치 기록/ring buffer 20건 제한/미등록 엔진 빈 목록/engine_name 생략 시 전체 조회/reason 참고 문구가 evidence·순위를 바꾸지 않음), `tests/runtime/engine/test_managed_engine_runtime.py`(동일 시나리오 5건) 추가. `pytest` 1384개(신규 11개, 회귀 없음)/`ruff`/`mypy`(236 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Workspace Reflection & Continuous Improvement 서술 추가.

## ADR-0100: Goal-Aware Decision Policy — Cost/Quality/Latency 우선순위를 조정하는 Policy 계층 (Milestone 82)

- 배경: 사용자가 "M80 Autonomous Decision Engine이 모든 Task를 동일한 기준으로 처리하지 않고, 사용자가 지정한 목표(Goal)에 따라 의사결정 우선순위를 동적으로 변경"하는 M82를 요청했다. 최신 main(M81/ADR-0099까지 반영) 기준으로 조사한 결과, `decide_engine()`(M80)은 항상 `recommend_engine()`(M79)의 1순위를 그대로 채택하고, `recommend_engine()`은 항상 `EngineSelectionPolicy.select()`(비용 최소 고정)의 결과만 반환해, Task/호출자가 "품질을 더 중시" 또는 "latency를 더 중시" 같은 의도를 표현할 방법이 전혀 없었다.
- 사용자 승인(AskUserQuestion, 4회):
  1. Goal 노출 위치 — **`recommend_engine()`/`decide_engine()`에만 선택적 `goal` 키워드 파라미터(권장안 채택)**: `run()`/`run_parallel()`/`_select()`는 전혀 건드리지 않는다. 기존 호출자는 전혀 영향받지 않는다(100% 하위 호환). 이 결정의 직접적 귀결로, M80의 "`decide_engine()`은 항상 `run()`과 같은 엔진을 반환한다"는 보장은 `goal`이 `BALANCED`(기본값)일 때만 유효해진다 — `EngineRuntime`에 전역 `set_goal()` 상태를 추가해 `run()`까지 참조하게 하는 대안은 "Routing 알고리즘은 변경하지 않는다"는 요구와 정면으로 충돌해 기각했다.
  2. Goal별 순위 결정 방식 — **우선순위 튜플 정렬(권장안 채택)**: `(1차키, 2차키, 3차키, engine_name)` tie-break 튜플로 정렬한다 — `COST_OPTIMIZED=(cost, -quality, latency, name)` / `QUALITY_OPTIMIZED=(-quality, cost, latency, name)` / `LATENCY_OPTIMIZED=(latency, cost, -quality, name)` / `BALANCED`=기존 `EngineSelectionPolicy.select()` 그대로(변경 없음). `quality`/`latency`는 M81이 이미 정의한 우선순위(execution_memory > benchmark > reliability, execution_memory > benchmark)로 대표값 하나를 뽑는다(`_representative_success_rate()`/`_representative_latency_seconds()`로 신설·공유). 정규화 가중합 점수 방식은 cost(USD)/quality(0~1)/latency(초)의 단위가 전혀 달라 임의의 정규화 기준(최대/최소 범위)을 새로 발명해야 해(YAGNI 위반) 기각했다.
  3. Budget 필터링 — **기존과 동일하게 적용(권장안 채택)**: Goal 기반 재정렬 전에 `budget_policy_engine.check()`로 예산 초과 후보를 먼저 제외한다(`InMemoryEngineSelectionPolicy`와 동일한 필터링 로직 재사용) — Goal이 필터링 규칙까지 우회하면 예산 정책이 조용히 무력화되는 새로운 불일치가 생긴다.
  4. `decide_engine()`의 "run()과 항상 같은 엔진" 보장 범위 — **`BALANCED`일 때만 보장(권장안 채택)**: 다른 goal을 명시하면 `run()`과 다른 엔진을 반환할 수 있음을 `reason`에 명시적으로 남긴다("(BALANCED가 아니므로 run()과 다른 엔진일 수 있음)"). `run()`에도 `goal`을 추가해 항상 동기화하는 대안은 `_select()`까지 수정해야 해 "Routing 알고리즘 무변경" 요구와 범위가 커진다는 이유로 기각했다.
- 결정:
  1. `src/ai_workspace/domain/decision_goal.py`에 `DecisionGoal`(Enum: `COST_OPTIMIZED`/`QUALITY_OPTIMIZED`/`LATENCY_OPTIMIZED`/`BALANCED`)을 신설한다. 새 Core Domain Interface가 아니라 `TaskStatus`/`EngineSessionStatus` 등 기존 Enum과 동일한 방식의 domain 값 타입이다.
  2. `EngineRuntime`(interface)의 `recommend_engine()`/`decide_engine()`에 `goal: DecisionGoal = DecisionGoal.BALANCED` 키워드 전용 파라미터를 추가한다(M70/M77/M79/M80/M81과 동일한 방식의 계약 확장 — 새 Core Domain Interface 아님).
  3. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에서 `recommend_engine()`이 `goal is DecisionGoal.BALANCED`(기본값)면 기존 코드를 한 줄도 거치지 않고 그대로 실행한다(신규 분기 완전 우회). 그 외 goal이면(그리고 `engine_selection_policy` 주입 시에만) `_build_candidates()`가 만든 후보에 Budget 필터링을 적용한 뒤 `_goal_rank_key()`(신규 private 메서드)로 정렬해 상위 `top_n`을 `EngineRecommendation`으로 반환한다(`_evidence_snapshot()`(M79/M81)을 그대로 재사용해 `evidence`/`confident`를 채움). `engine_selection_policy` 미주입 시(첫 매칭 경로)에는 `goal`과 무관하게 관여하지 않는다.
  4. `decide_engine()`은 `goal`을 `recommend_engine()`에 그대로 전달하고, `goal is not DecisionGoal.BALANCED`면 반환된 `reason`에 "run()과 다른 엔진일 수 있음" 문구를 덧붙인다. 추천이 아예 없으면(후보 없음) 기존과 동일하게 `_select()`/`_require_adapter()`로 fallback한다(이 fallback은 `goal`과 무관, 항상 기존 `EngineSelectionPolicy` 결과).
  5. `_representative_success_rate(evidence)`/`_representative_latency_seconds(evidence)`를 모듈 레벨 함수로 신설해 M81 `_record_reflection()`의 인라인 계산을 대체(리팩터링, 동작 무변경)하고 M82 `_goal_rank_key()`가 동일 계산을 재사용하게 한다 — 우선순위 계산 로직을 한 곳에서만 정의한다(DRY).
  6. `RecoveringEngineRuntime`(데코레이터)은 내부 Runtime에 그대로 위임하고, 테스트 대역 4곳(`FakeEngineRuntime`/`RecordingEngineRuntime`/`SpyEngineRuntime`/`ScriptedEngineRuntime`)에 `goal` 파라미터를 추가하되 동작은 그대로 둔다(Fake는 goal을 사용하지 않음).
  7. `_select()`/`_build_candidates()`/재정렬 메서드들(`_reorder_by_diversity()`/`_reorder_by_benchmark()`/`_reorder_by_execution_memory()`)과 `EngineSelectionPolicy`는 단 한 줄도 수정하지 않는다 — Goal은 그 결과 위에서만 동작하는 순수 Policy 계층이다.
- 대안:
  1. `EngineRuntime`에 전역 `set_goal()`/`current_goal` 상태를 추가해 `run()`까지 참조 — 기각(사용자가 파라미터 방식 선택): "Routing 알고리즘은 변경하지 않는다"는 요구에 직접 위배되고, 전역 가변 상태가 동시 호출 간 의도치 않은 간섭을 일으킬 위험이 있다.
  2. 정규화 가중합 점수(`score = w1*cost_norm + w2*quality_norm + w3*latency_norm`) — 기각(사용자가 우선순위 튜플 선택): 단위가 다른 세 값을 하나의 스칼라로 합치려면 임의의 정규화 범위/가중치 상수를 새로 발명해야 해 "새 알고리즘이 아니라 우선순위 조정"이라는 요구와 결이 다르다.
  3. Goal 기반 재정렬에서 Budget 필터링 생략 — 기각(사용자가 기존과 동일 적용 선택): Goal이 예산 정책을 조용히 우회하면 "기존 의사결정 요소의 우선순위만 조정한다"는 원칙에서 벗어나 새로운 동작(예산 무시)이 추가된다.
  4. `run()`에도 `goal`을 추가해 항상 `decide_engine()`과 동기화 — 기각(사용자가 `BALANCED`일 때만 보장 선택): `_select()`까지 수정해야 하므로 "기존 Routing 알고리즘 무변경, Policy Layer만 추가"라는 요구의 범위를 벗어난다.
- 이유: `_build_candidates()`(M64~M78)가 이미 Cost/Reliability/Benchmark/Diversity/Workflow Learning 신호를 전부 계산해 후보로 만들어 두었고, M79의 `_evidence_snapshot()`/M81의 `_representative_success_rate()`/`_representative_latency_seconds()`가 그 신호들을 대표값으로 이미 정리해 두었으므로, Goal은 이 기존 값들을 다른 우선순위로 정렬만 하면 된다 — 새 측정·새 알고리즘·새 상태 없이 "우선순위를 조정하는 Policy"라는 요구를 문자 그대로 만족한다. `BALANCED`를 "새 분기 자체가 실행되지 않는 것"으로 정의해, "Goal이 지정되지 않으면 기존 동작과 100% 동일해야 한다"는 요구를 코드 경로 수준에서 구조적으로 보장했다.
- 결과/영향: `src/ai_workspace/domain/decision_goal.py` 신설(`DecisionGoal`). `src/ai_workspace/interfaces/engine_runtime.py`(`recommend_engine()`/`decide_engine()`에 `goal` 파라미터 추가). `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`(`_representative_success_rate()`/`_representative_latency_seconds()`/`_goal_rank_key()`/`_goal_reason()`/`_recommend_by_goal()` 추가, `_record_reflection()`이 새 헬퍼 재사용하도록 리팩터링, `recommend_engine()`/`decide_engine()`에 `goal` 분기 추가, 클래스 docstring 갱신). `src/ai_workspace/runtime/engine/recovering_engine_runtime.py`(위임 확장). 테스트 대역 4곳에 `goal` 파라미터 추가(동작 무변경). 새 Core Domain Interface 없음(기존 `EngineRuntime`에 선택적 키워드 파라미터만 추가, 100% 하위 호환). `tests/runtime/engine/test_engine_runtime.py`(신규 9건: BALANCED가 생략과 동일/QUALITY_OPTIMIZED가 신뢰도 우선/COST_OPTIMIZED가 비용 우선/LATENCY_OPTIMIZED가 latency 우선/Budget 필터링 적용·전체 초과 시 빈 목록/정책 미주입 시 미관여/top_n 순위/decide_engine의 run() 불일치 명시), `tests/runtime/engine/test_managed_engine_runtime.py`(동일 시나리오 7건) 추가. `pytest` 1400개(신규 16개, 회귀 없음)/`ruff`/`mypy`(237 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`)에 Goal-Aware Decision Policy 서술 추가.

## ADR-0101: Persistent Learning Memory — 학습 상태를 단일 JSON 파일로 영속화 (Milestone 83)

- 배경: 사용자가 "M65~M82에서 학습한 Engine Reliability, Benchmark, Workflow Learning, Reflection, Recommendation 데이터를 세션 종료 후에도 유지"하는 M83을 요청했다. 최신 main(M82/ADR-0100까지 반영) 기준으로 조사한 결과, `InMemoryEngineRuntime`/`ManagedEngineRuntime`의 `_engine_reliability`(M65)/`_execution_memory`(M69)/`_consensus_agreement`(M70)/`_reflections`(M81)와 `InMemoryWorkflowEngine`의 `_order_stats`(M71)가 모두 in-process dict/deque로만 존재해 프로세스 종료(CLI는 매 호출마다 새 프로세스) 시 사라졌다. `EngineBenchmarkProfile`(M77)/`EngineRecommendation`(M79)은 그 자체로 저장되는 상태가 아니라 위 4가지 원시 데이터를 읽기 전용으로 재조합한 파생값이라 별도 영속화 대상이 아님을 확인했다. 또한 프로덕션 Composition Root에서 `EngineRuntime`이 실제로 배선된 곳은 CLI(`cli/main.py`)뿐이었다(각 명령이 독립 프로세스).
- 사용자 승인(AskUserQuestion, 4회):
  1. 연결 지점 — **`WorkspaceCore.__init__`/`shutdown()`(권장안 채택)**: `WorkspaceCore`에 이미 "종료" 의미를 가진 `shutdown()`이 있고, 생성자가 `engine_runtime`/`workflow_engine`을 이미 주입받으므로 대칭적인 지점이다. CLI 조립 함수에서 외부로 처리하는 대안은 향후 다른 진입점(웹 서버 등)에서 중복 구현이 필요해 기각.
  2. API 표면 — **기존 `EngineRuntime`/`WorkflowEngine` 추상 인터페이스 확장(권장안 채택)**: M70/M77/M79~M82와 동일한 방식으로 `export_learning_state()`/`import_learning_state()`를 추가한다. 구체 클래스에만 추가하는 대안은 `WorkspaceCore`/Composition Root가 구체 타입을 알아야 해(isinstance 필요) §8 규칙 3("Interfaces에만 의존")과 충돌할 위험이 있어 기각.
  3. 파일 구성 — **단일 JSON 파일로 통합(권장안 채택)**: `<data_dir 상위>/.ai-workspace-data/learning_state.json` 하나에 `engine_runtime`/`workflow_engine` 두 스냅샷을 함께 담는다. `FileMemoryEngine`(M50)과 동일한 패턴(JSON 직렬화, `base_dir` 생성자 주입)이다.
  4. 실패 처리 — **조용히 무시하고 in-process 기본값으로 fallback(권장안 채택)**: 파일 손상/권한 오류/디스크 문제 등 모든 I/O·파싱 예외를 `FileLearningStateStore.save()`/`load()`가 catch해 삼킨다. 새 로깅/예외 인프라를 추가하지 않는다(YAGNI).
- 결정:
  1. `EngineRuntime`(interface)에 `export_learning_state() -> dict[str, Any]`/`import_learning_state(state: dict[str, Any]) -> None`을, `WorkflowEngine`(interface)에도 동일한 시그니처를 추상 메서드로 신설한다(새 Core Domain Interface 아님, 기존 계약 확장).
  2. `InMemoryEngineRuntime`/`ManagedEngineRuntime`에서 `_engine_reliability`/`_execution_memory`/`_consensus_agreement`/`_reflections`를 JSON 직렬화 가능한 dict로 내보내고 되돌린다. `(required_capabilities, engine_name)` 튜플 키는 JSON dict 키가 될 수 없으므로 `capabilities`(정렬 리스트)/`engine_name` 필드를 가진 레코드 리스트로 표현한다. `import_learning_state()`는 기존 상태를 대체한다(병합이 아니다). 세션·동시성 상태(`_engines`/`_task_status`/`_max_concurrency`/`_in_flight`)는 "학습 데이터"가 아니므로 포함하지 않는다.
  3. `InMemoryWorkflowEngine`에서 `_order_stats`(이중 중첩 `frozenset`/`tuple` 키 dict)를 `task_ids`(정렬 리스트)/`order`(리스트) 필드를 가진 평평한 레코드 리스트로 표현한다.
  4. `src/ai_workspace/storage/file_learning_state_store.py`에 `FileLearningStateStore`(새 Core Domain Interface 아님 — `EngineRuntime`/`WorkflowEngine` 어느 쪽도 구현하지 않는 순수 I/O 헬퍼 클래스)를 신설한다. `save(engine_runtime, workflow_engine)`/`load(engine_runtime, workflow_engine)`가 두 `export_learning_state()`를 하나의 JSON에 쓰고, 두 `import_learning_state()`로 되돌린다 — 모든 예외를 catch해 조용히 무시한다.
  5. `WorkspaceCore.__init__`에 `restore_learning_state: Callable[[], None] | None = None`/`persist_learning_state: Callable[[], None] | None = None`을 추가한다. `restore_learning_state`는 `__init__` 끝에서, `persist_learning_state`는 기존 `shutdown()` 시작에서 각각 주어졌을 때만 호출한다 — `WorkspaceCore`는 §8 규칙 3("Workspace Core → Agent Runtime, Engine Runtime, **Interfaces**에만 의존")에 따라 구체 저장소(`FileLearningStateStore`)를 알면 안 되므로, 콜백을 인자 없는 클로저로 받아 "무엇을 어떻게 저장하는지"를 완전히 모른 채로 "언제 호출할지"만 책임진다. 두 콜백 모두 기본값 `None`이면 이 두 시점에 아무 일도 일어나지 않아 기존 동작과 100% 동일하다.
  6. CLI `_build_workspace_core()`가 Composition Root로서 `ManagedEngineRuntime`/`InMemoryWorkflowEngine`/`FileLearningStateStore(Path(data_dir).parent / ".ai-workspace-data")`를 조립하고, 이미 만들어 둔 두 컴포넌트를 클로저로 묶은 람다를 `restore_learning_state`/`persist_learning_state`로 전달한다. `main()`은 `core.shutdown()`을 `finally` 블록에서 호출해 예외가 나도 저장이 시도되게 한다.
  7. `RecoveringEngineRuntime`(데코레이터)은 내부 Runtime에 그대로 위임하고, 테스트 대역들(`FakeEngineRuntime`/`RecordingEngineRuntime`/`SpyEngineRuntime`/`ScriptedEngineRuntime`/`FakeWorkflowEngine`)에 계약을 만족하는 최소 구현을 추가한다.
- 대안:
  1. CLI 조립 함수에서 외부로 직접 load/save 호출 — 기각(사용자가 WorkspaceCore 연결 선택): `WorkspaceCore` 책임(초기화/종료)과 자연스럽게 맞지 않고, 향후 다른 Composition Root(웹 서버 등)가 생기면 동일 로직을 중복 구현해야 한다.
  2. 구체 클래스(`InMemoryEngineRuntime`/`ManagedEngineRuntime`/`InMemoryWorkflowEngine`)에만 export/import 메서드 추가, 추상 인터페이스 무변경 — 기각(사용자가 인터페이스 확장 선택): `WorkspaceCore`/Composition Root가 주입받은 값의 구체 타입을 알아야 호출할 수 있어(`isinstance` 필요) 다형성이 깨지고 §8 규칙 3과 결이 다르다.
  3. 관심사별 별도 파일(`engine_learning_state.json`/`workflow_learning_state.json`) — 기각(사용자가 단일 파일 선택): 파일 수만 늘어날 뿐 저장/복원을 원자적으로 처리하려는 이번 요구에 이점이 없다.
  4. 실패 시 stderr 경고 출력 — 기각(사용자가 조용한 fallback 선택): "기능 저하 없이 동작해야 한다"는 요구를 코드 자체로 강제하려면 새 로깅 경로를 추가하지 않는 편이 더 단순하다(필요해지면 Observability 계층에서 별도로 다룰 수 있다).
  5. `WorkspaceCore`가 `FileLearningStateStore`를 직접 생성자로 주입받기 — 기각: §8 규칙 3("Interfaces에만 의존")을 직접 위배한다(`FileLearningStateStore`는 구체 storage 클래스이지 Interface가 아니다). 콜백 클로저 방식으로 이 경계를 지켰다.
- 이유: M65~M82가 이미 확립한 4가지 값 객체(`EngineReliabilityStat`/`EngineExecutionMemoryStat`/`ConsensusAgreementStat`/`ReflectionReport`)와 M71의 `WorkflowOrderStat`을 그대로 JSON으로 인코딩·디코딩하는 것만으로 "영속화"를 구현할 수 있다 — 새 Learning 데이터나 새 알고리즘이 전혀 필요 없다. 저장소를 인터페이스가 아닌 콜백으로 `WorkspaceCore`에 연결해, "Workspace 시작/종료에 연결"이라는 요구와 "Workspace Core는 구체 클래스를 모른다"는 기존 architecture 원칙을 동시에 만족시켰다. 모든 I/O 실패를 삼키는 것으로 "저장 실패 시 기존 in-process 동작 즉시 fallback"을 코드 경로 자체로 보장한다(예외가 상위로 전파될 경로가 없다).
- 결과/영향: `src/ai_workspace/interfaces/engine_runtime.py`/`workflow_engine.py`(`export_learning_state()`/`import_learning_state()` 추상 메서드 추가). `src/ai_workspace/runtime/engine/engine_runtime.py`/`managed_engine_runtime.py`(구현 추가). `src/ai_workspace/engines/workflow_engine.py`(구현 추가). `src/ai_workspace/runtime/engine/recovering_engine_runtime.py`(위임 추가). `src/ai_workspace/storage/file_learning_state_store.py` 신설(`FileLearningStateStore`). `src/ai_workspace/core/workspace_core.py`(`restore_learning_state`/`persist_learning_state` 콜백 파라미터 추가, `__init__`/`shutdown()` 연결). `src/ai_workspace/cli/main.py`(`_build_workspace_core()`가 저장소 조립·콜백 연결, `main()`이 `finally`에서 `core.shutdown()` 호출). 테스트 대역 5곳에 최소 구현 추가. 새 Core Domain Interface 없음(기존 `EngineRuntime`/`WorkflowEngine`에 메서드 추가 + 순수 I/O 헬퍼 클래스 1개, 100% 하위 호환 — 콜백 기본값 `None`이면 `WorkspaceCore` 동작 무변경). `tests/runtime/engine/test_engine_runtime.py`(신규 4건), `tests/runtime/engine/test_managed_engine_runtime.py`(신규 3건), `tests/runtime/engine/test_recovering_engine_runtime.py`(신규 1건), `tests/engines/test_workflow_engine.py`(신규 3건), `tests/storage/test_file_learning_state_store.py`(신규 5건: 미존재 파일/왕복/디렉터리 자동 생성/손상 파일 fallback/쓰기 불가 fallback), `tests/core/test_workspace_core.py`(신규 3건: 콜백 호출 시점 검증, 미지정 시 SpyEngineRuntime의 AssertionError로 미호출 증명), `tests/cli/test_main.py`(신규 2건: `_build_workspace_core()` 왕복, 최초 실행 시 빈 상태) 추가. `pytest` 1421개(신규 21개, 회귀 없음)/`ruff`/`mypy`(238 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.3 Workspace Core·§3.9 Engine Runtime 절 및 §7 인터페이스 표(`EngineRuntime`/`WorkflowEngine`)에 Persistent Learning Memory 서술 추가.

## ADR-0102: Multi-Agent Negotiation — 실행 전 선호안 다수결 조율 계층 (Milestone 84)

- 배경: 사용자가 "여러 Agent가 동일한 Task 또는 공유 자원(Provider/Model/Workflow)을 사용할 때 독립적으로 결정하지 않고, 협상(Negotiation)을 통해 역할과 실행 계획을 조율하는 최소 범위"인 M84를 요청했다. 최신 main(M83/ADR-0101까지 반영) 기준으로 조사한 결과, `EngineRuntime.decide_engine()`(M80)은 항상 단일 호출자의 관점에서 "하나의" 최적 엔진을 결정할 뿐 여러 Agent의 서로 다른 선호를 입력으로 받는 개념이 전혀 없었고, `AgentManager`(T1-18)는 생성/상태 전이만 책임져 Task/자원 배정에 관여하지 않았으며, `WorkflowEngine.plan()`은 단일 결정론적 순서만 반환했다 — "여러 제안을 조율해 하나로 합의"하는 계층은 어디에도 없었다.
- 사용자 승인(AskUserQuestion, 4회):
  1. 배치 위치 — **독립 오케스트레이션 클래스(권장안 채택)**: 새 Core Domain Interface 없이 `runtime/negotiation/negotiation_coordinator.py`에 순수 클래스로 추가하고, `EngineRuntime`/`WorkflowEngine`을 생성자로 주입받아 재사용한다. `WorkspaceCore`/`AgentManager`에는 자동 배선하지 않는다(옵트인) — `LearningRuntimeAnalyzer`(M54)와 동일한 패턴. `EngineRuntime` 추상 메서드 확장안은 "여러 Agent의 제안"이라는 개념이 `EngineRuntime`의 책임 밖이라 기각.
  2. 제안(Proposal) 형식 — **호출자가 조립한 `AgentProposal` 목록 전달(권장안 채택)**: `NegotiationCoordinator`는 Agent 인스턴스를 알지 못하며, `agents/` 패키지도 전혀 수정하지 않는다(결합 최소화, ADR-0080/0081 원칙과 동일). Agent 클래스에 `propose()`를 신설하는 대안은 6개 Agent 클래스를 모두 수정해야 해 결합도가 높아진다는 이유로 기각.
  3. 충돌 해결 — **다수결 + `Agent.priority`(M57) tie-break(권장안 채택)**: `EngineSelectionPolicy` 비용 재평가안은 Agent의 실제 투표를 무시해 "협상"이 아니라 기존 라우팅 재사용에 가까워 기각.
  4. Fallback 범위 — **전체 all-or-nothing(권장안 채택)**: engine/model/workflow 순서 중 하나라도 합의에 실패하면 전체를 `EngineRuntime.decide_engine()`/`WorkflowEngine.plan()`으로 즉시 전환한다. 구성 요소별 개별 fallback안은 "부분 협상+부분 단독 결정"이 섞여 결과 해석이 복잡해진다는 이유로 기각.
- 결정:
  1. `src/ai_workspace/domain/negotiation.py`에 `AgentProposal`(frozen dataclass: `agent_id`/`engine_name`/`model: str | None = None`/`priority: int = 0`/`preferred_workflow_order: tuple[str, ...] | None = None`)과 `NegotiatedPlan`(frozen dataclass: `decision: EngineSelectionDecision`/`workflow_order: list[str] | None`/`consensus_reached: bool`/`reason: str`)을 신설한다. `NegotiatedPlan.decision`은 새 반환 타입을 만들지 않기 위해 기존 M17 `EngineSelectionDecision`을 그대로 재사용한다.
  2. `src/ai_workspace/runtime/negotiation/negotiation_coordinator.py`에 `NegotiationCoordinator(engine_runtime: EngineRuntime, workflow_engine: WorkflowEngine | None = None)`를 신설한다. `negotiate(task, proposals, required_capabilities=frozenset(), *, goal=DecisionGoal.BALANCED, workflow=None) -> NegotiatedPlan`가 유일한 public 메서드다.
  3. `engine_name` 다수결: `Counter`로 최다 득표를 집계해 단독 최다 득표자가 있을 때만 채택한다(동률이면 협상 실패로 즉시 전체 fallback — priority로 임의로 동률을 깨지 않는다, "동률을 강제로 깨는 것은 합의가 아니다"). 채택된 `engine_name` 그룹 내에서 `model`이 갈리면 `AgentProposal.priority`(낮을수록 우선, `Agent.priority`(M57)와 동일 의미)가 가장 낮은 제안의 `model`을 대표값으로 쓴다.
  4. Workflow 순서: `workflow`가 주어지고 `preferred_workflow_order`를 명시한 제안이 하나도 없으면 충돌이 아니므로 `WorkflowEngine.plan(workflow)`를 그대로 채택한다(협상 대상이 아님, `consensus_reached`는 engine 결과만으로 결정). 하나 이상 있으면 동일하게 다수결(동률이면 전체 fallback)로 후보를 고르고, 채택 전 `workflow.dependencies`를 실제로 만족하는지 재검증한다(M72 `plan()`의 "현재 dependency 재검증 후 채택" 원칙과 동일 로직 재사용, 새 알고리즘 아님) — 위반하면 전체 fallback.
  5. Fallback: `_fallback()`이 `EngineRuntime.decide_engine(task, required_capabilities, goal=goal)`과(주어졌다면) `WorkflowEngine.plan(workflow)`를 그대로 호출해 `NegotiatedPlan(consensus_reached=False, ...)`을 만든다 — 새 로직 없이 기존 두 계약을 그대로 재사용한다.
  6. `NegotiationCoordinator`는 호출 간 아무 상태도 in-process로 보관하지 않는다(순수 함수형 오케스트레이션, `negotiate()` 인자로만 값이 오간다) — "상태는 in-process로만 관리, 영속화 제외" 요구를 상태 자체를 두지 않는 것으로 만족시켰다.
  7. `WorkspaceCore`/`AgentManager`/`EngineRuntime`/`WorkflowEngine`의 기존 public API는 단 한 글자도 수정하지 않는다 — `NegotiationCoordinator`는 호출자가 명시적으로 생성·호출해야만 존재하는 완전한 옵트인 계층이다.
- 대안:
  1. `EngineRuntime.negotiate_engine()` 추상 메서드 신설 — 기각(사용자가 독립 클래스 선택): "여러 Agent의 제안을 입력받아 조율"이라는 책임은 "하나의 호출자 관점에서 최적 엔진을 고르는" `EngineRuntime`의 기존 책임 범위 밖이라 계약이 비대해진다.
  2. Agent 클래스에 `propose()` 메서드 신설 — 기각(사용자가 호출자 조립 방식 선택): `PlanningAgent`/`CodingAgent`/`ReviewAgent`/`DocumentationAgent`/`ShellAgent`/`CoordinatorAgent` 6개를 모두 수정해야 하고, `NegotiationCoordinator`가 Agent 구현 세부사항에 결합된다.
  3. `EngineSelectionPolicy` 비용 기준으로 제안을 재평가 — 기각(사용자가 다수결 선택): Agent의 실제 투표수를 무시하고 비용만 보면 기존 `_select()`/`decide_engine()` 라우팅의 재적용에 가까워 "협상"이라는 요구의 본질(참여자 합의)을 반영하지 못한다.
  4. 구성 요소별(engine/model/workflow) 개별 fallback — 기각(사용자가 all-or-nothing 선택): "engine은 협상 결과, workflow는 단독 결정"처럼 결과가 섞이면 `consensus_reached` 단일 필드로 상태를 표현할 수 없어 호출자의 해석이 복잡해진다.
- 이유: `EngineSelectionDecision`(M17)/`WorkflowEngine.plan()`(T2-03)/`EngineRuntime.decide_engine()`(M80)이 이미 "단일 결정"을 만드는 데 필요한 모든 메커니즘을 갖추고 있으므로, 협상은 그 위에 다수결이라는 최소 규칙 하나만 추가하고 실패 시 그 기존 메커니즘으로 그대로 위임하면 충분했다 — 새 알고리즘도, 새 실행 경로도, 새 Core Domain Interface도 필요하지 않았다. 독립 클래스+명시적 호출 방식을 선택해 "협상은 실행 전 단계에서만 수행하며 기존 실행 흐름을 변경하지 않는다"는 요구를 코드 구조 자체로 보장했다(자동 배선이 전혀 없으므로 협상을 쓰지 않는 기존 호출 경로는 이 변경을 알지도 못한다).
- 결과/영향: `src/ai_workspace/domain/negotiation.py` 신설(`AgentProposal`/`NegotiatedPlan`). `src/ai_workspace/runtime/negotiation/negotiation_coordinator.py` 신설(`NegotiationCoordinator`). 기존 파일 수정 없음(모두 신규 파일, `EngineRuntime`/`WorkflowEngine`/`AgentManager`/`WorkspaceCore` 계약·구현 무변경). 새 Core Domain Interface 없음. `tests/runtime/negotiation/test_negotiation_coordinator.py` 신규 7건(다수결 채택/제안 없음 fallback/engine 동률 fallback/model tie-break/workflow 다수결 채택/dependency 위반 fallback/미제안 시 plan() 재사용) 추가. `pytest` 1428개(신규 7개, 회귀 없음)/`ruff`/`mypy`(241 source files) 전부 통과. `docs/ARCHITECTURE.md` §3.9 Engine Runtime 절에 Multi-Agent Negotiation 서술 추가(새 Interface가 아니므로 §7 인터페이스 표는 무변경).
