...
- 목적: M44까지 완성된 Recommendation(M35)→Adaptation(M42)→Explainability(M44)→Orchestration(M43)→Execution(M36)→Memory(M39)→Experience(M40) 파이프라인이 사후(Vault 문서/pytest 로그)로만 확인 가능하던 것을, 새 AI 판단·자동화 추가 없이 Claude Code 세션 안에서 실시간 StatusLine으로 반영(Phase 1, Claude Code 내부 표시만)
- 결정: T01 Domain Analysis로 `Observability`가 §13.2 4개 핵심 Domain(Intelligence/Memory/Execution/Guardian) 어디에도 정확히 해당하지 않음을 확인 — Intelligence와 같이 Read Only지만 새 판단(분석·요약)을 하지 않고 이미 계산된 값의 존재 여부만 반영한다는 점이 다름. 재사용 사례 1건(StatusLine)뿐이라 `Adaptation`/`Explainability`/`Orchestration`과 동일 급의 **Behavioral Concept**로 §13.3에 신규 등재(1급 Domain 승격 보류). `observability/`(신규 패키지) — `WorkspaceRuntimeSnapshot`(읽기 전용 Runtime 모델) + 3개 Analyzer(`ClaudeRuntimeAnalyzer`/`PipelineStageAnalyzer`/`WorkspaceInfoAnalyzer`) + `RuntimeSnapshotService` + `StatusLineRenderer`. `VaultAdapter.report_last_modified()` 1개 메서드만 확장(새 Adapter 없음). Adaptation/Orchestration은 별도 Vault 산출물이 없어 `STRUCTURAL_INCLUDED`, Memory(M39)는 `InMemoryMemoryEngine` 기반이라 비영속이라 별도 프로세스인 StatusLine에서 조회 불가해 `NOT_OBSERVABLE`로 정직하게 표시 — 실제 관측 가능한 4단계(Recommendation/Explainability/Execution/Experience)만 Vault 문서 존재 여부로 판정, 값을 지어내지 않음. Claude Runtime 정보(Model/Effort/Context 사용량/Tokens)는 Claude Code StatusLine stdin JSON의 공식 문서화된 필드만 그대로 옮김. `.claude/settings.json`(신규)에 StatusLine 실배선
- 영향: `observability/`(신규 패키지 7개 파일: `snapshot.py`/`claude_runtime_analyzer.py`/`pipeline_stage_analyzer.py`/`workspace_info_analyzer.py`/`runtime_snapshot_service.py`/`statusline_renderer.py`/`statusline_main.py`), `integration/vault_adapter.py`(`report_last_modified()` 1개 메서드 추가), `.claude/settings.json`(신규), `docs/ARCHITECTURE.md` §13.3/§13.4/§3.37(신규)/헤더 상태 갱신. Vault 신규 발행 없음(Phase 1은 StatusLine 전용). 새 Core Domain Interface/Adapter 없음(27종 유지). `pytest` 1090개(17개 신규)·ruff·mypy 전부 클린, Guardian all_passed 유지, `build_app()` 실제 조립 스모크 테스트 통과. 상세는 [[Architecture Overview]]

## ADR-0063: Workspace Observability 확장 — Execution Environment(Git/Guardian/Vault/MCP) 관찰 (Milestone 45 확장)

- 목적: M45(Claude Runtime + Pipeline Observability)를 "AI Workspace Runtime"뿐 아니라 "Execution Environment"(Git/Guardian/Vault/MCP, 특히 Obsidian MCP)까지 확장 관찰. 구현 전 Claude Code/MCP 공식 문서로 관측 가능한 필드를 먼저 조사하고, 추정값 없이 사용 가능한 항목만 구현하며 관측 불가 항목은 이유와 함께 Not Available로 표시하도록 사용자가 조건 명시
- 결정: T01 Domain Analysis로 이 확장이 새 Domain도 새 Behavioral Concept도 아님을 확인 — ADR-0062가 §13.3에 이미 등재한 `Observability`를 그대로 확장(관찰 대상만 늘어남). T02 Architecture Review(공식 문서 조사)로 StatusLine stdin JSON에 MCP 필드가 전혀 없음을 확인, `.mcp.json`(설정된 서버 목록)과 `claude mcp list`(공식 CLI, 문서화된 상태 기호만 매칭, JSON 옵션 없음)만 사용하기로 결정. `pytest`/`ruff`/`mypy`/Coverage 전체 재실행은 지연 위험이 커 금지 — `guardian.checker.evaluate()`(AST 기반 순수 평가)만 재사용. T03 Detailed Design으로 관측 가능(Git 하위 명령/`.pytest_cache/lastfailed`/`VaultAdapter.report_last_modified()`/`.mcp.json`+`claude mcp list`)과 관측 불가(`ruff`/`mypy`/Coverage 재실행, MCP `active_server`/`available_tools`/`last_mcp_call`/`last_mcp_error`, Vault `current_pr`, Workspace `current_task`)를 명확히 구분. T04로 `GitRuntimeAnalyzer`/`GuardianRuntimeAnalyzer`/`VaultRuntimeAnalyzer`/`McpRuntimeAnalyzer`(신규 4개) 구현 — `git fetch`/GitHub API 등 네트워크 호출 없음, 재실행 없음 원칙 유지
- 영향: `observability/`(신규 4개 파일: `git_runtime_analyzer.py`/`guardian_runtime_analyzer.py`/`vault_runtime_analyzer.py`/`mcp_runtime_analyzer.py`), `observability/snapshot.py`(`GitRuntimeInfo`/`GuardianRuntimeInfo`/`VaultRuntimeInfo`/`McpRuntimeInfo` 추가, `WorkspaceInfo.current_task` 필드 추가)/`runtime_snapshot_service.py`/`statusline_renderer.py`(확장), `docs/ARCHITECTURE.md` §3.38(신규)/헤더 상태 갱신. Vault 신규 발행 없음(StatusLine 전용 유지). 새 Core Domain Interface/Adapter 없음(27종 유지). `pytest` 1108개(18개 신규)·ruff·mypy 전부 클린, Guardian all_passed 유지, `build_app()` 실제 조립 스모크 테스트 통과, `statusline_main.py` 실제 stdin JSON 수동 검증 완료. 상세는 [[Architecture Overview]]
