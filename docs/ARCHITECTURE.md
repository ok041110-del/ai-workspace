...
- **M41 Architecture Guardian — 완료(2026-07-30, ADR-0056)**: 위 세
  ...
  패키지 하나로 통합했다(§3.33).
- **M48 Automation Foundation — 완료(2026-08-01, ADR-0065)**: M41
  Architecture Guardian이 평가·공표만 할 뿐 M36~M38 자동 실행
  경로(`AutomationScheduler`→`RecommendationOrchestrationService`)
  어디에도 연결돼 있지 않던 공백을, Learning Engine을 먼저
  설계하는 대신 우선 메웠다(사용자 지시 — 아직 신호가 쌓이지 않는
  상태에서 Learning을 설계하는 것은 YAGNI 위반). `ExecutionGate`
  에 Guardian 위반 시 Execution만 차단하는 선택적 판정을 추가하고
  (Recommendation/Adaptation/Explainability는 영향 없음), M45
...
- **M51 Learning Evolution — 완료(2026-08-01, ADR-0068)**: M49/M50의
  "실패율 100% + 표본 3건 이상" 규칙(전체 이력만 봄)에, 최근 추세
  기반 규칙을 **보완**으로 추가했다. `ExperienceStat`에
  `recent_failure_streak`(가장 최근 기록부터 거슬러 올라간 연속 실패
  횟수) 필드를 신설해, 최근 5건이 모두 실패면 전체 이력에 성공이
  섞여 있어도 추천을 보류한다(기존 규칙 무변경, OR 병존). 어느
...
- **M45-1 StatusLine Integration Fix — 완료(2026-08-01, ADR-0069)**:
  ...
  불가).
- **M52 Learning Weighting — 완료(2026-08-01, ADR-0070)**: M49(전체
  실패율)/M51(최근 연속 실패) 두 Rule의 OR 결합을 가중치 점수
  결합으로 확장했다. `signal_overall = failure_count/total`(표본
  3건 미만이면 0), `signal_recent = min(recent_failure_streak/5,
  1.0)`을 각각 0.6 가중치로 합산해 `score >= 0.6`이면 보류한다.
  가중치를 각 신호의 완전 포화값(1.0)과 같은 0.6으로 설정해, 신호
  하나가 완전히 1.0이면 그 신호만으로 이미 `score=0.6`이 성립 —
  기존 M49/M51 단일 규칙이 정확히 보존됨을 경계값으로 증명했다.
  최초 사용자 제안(가중치 0.5/0.5 + threshold 0.6)은 이 보존 조건을
  깨는 실제 회귀였음을 수학적으로 지적해 0.6/0.6으로 재확정했다.
...
- **Provider Concurrency Management(Milestone 74, ADR-0092)**: 여러
  Agent가 동시에 같은 Provider(등록된 Engine)를 선택하면 그 Provider의
  실제 동시 실행 한계(예: API rate limit)를 초과할 수 있는 위험이
  있었다 — 지금까지의 선택 로직(M64 비용/M65 신뢰도/M69 실행 이력)은
  모두 "어떤 엔진이 더 나은가"만 판단했지, "그 엔진이 지금 몇 개나
  동시에 실행 중인가"는 전혀 보지 않았다. `register_engine(name,
  adapter, *, max_concurrency: int | None = None)`(기본값 `None`=무제한)
  로 엔진별 동시 실행 상한을 선택적으로 지정할 수 있다. `InMemoryEngineRuntime`/
  `ManagedEngineRuntime` 둘 다 `_in_flight: dict[str, int]`(in-process
  카운터, `threading.Lock`으로 원자적 증감)로 "그 엔진에서 `run()`이
  세션을 만든 시점부터 실행이 끝날 때까지"의 개수를 추적한다.
  `_select()`/`_build_candidates()`(`ManagedEngineRuntime`은 `_require_
  adapter()`)가 한도에 도달한 엔진을 후보에서 제외해(capacity가 있으면
  기존과 동일하게 선택, 없으면 다른 후보로 자동 fallback), 후보가 전혀
  남지 않으면 기존과 동일한 `NoSuitableEngineError`를 던진다(새 예외
  타입 없음). 후보 필터링(스냅샷)과 실제 슬롯 획득(`_try_acquire()`)
  사이의 경쟁은 획득 실패 시 그 엔진을 제외하고 재선택하는 재시도
  루프(`_select_and_acquire()`/`_require_adapter_and_acquire()`)로
  안전하게 처리한다. `run_ensemble()`/`run_ensemble_auto()`(M62/M68)는
  호출자가 여러 Provider를 의도적으로 동시에 비교하는 기능이라 이
  범위에서 제외했다(YAGNI) — `max_concurrency`를 지정하지 않으면
...
- **Adaptive Load Balancing(Milestone 76, ADR-0094)**: M75의 부하 신호가
  raw `_in_flight` 개수였기 때문에, `max_concurrency`가 Provider마다
  다르면(예: 한도 10 중 3개 사용 vs 한도 2 중 1개 사용) 실제 여유를
  거꾸로 판단하는 문제가 있었다. `_reorder_by_diversity()`의 정렬 키를
  `_load_ratio()`(= `_in_flight / max_concurrency`, M74 상태만 읽는
  read-only 계산 — 새 상태 없음)로 개선했다 — 부하율이 동률이면(특히
  `max_concurrency`를 지정하지 않은 무제한 엔진끼리는 항상 0.0으로
  동률) M75가 제공하던 raw `_in_flight` 2차 tie-break로 그대로
  fallback해(`_load_rank()` 튜플 정렬) 기존 분산 동작을 100% 보존한다.
  `_reorder_by_diversity()`가 호출되는 위치(M74 capacity 필터링 이후,
  `_reorder_by_execution_memory()` 이전)·정렬 안정성·tie-break 전용
...
- **Engine Benchmark & Capability Profiling(Milestone 77, ADR-0095)**:
  ...
  방식으로 확장했을 뿐이다).
- **Adaptive Engine Benchmark Routing(Milestone 78, ADR-0096)**: M77이
  만든 `benchmark_profile()`은 순수 조회용이라 Routing에 실제로 반영되지
  않았다. `_build_candidates()`가 `_reorder_by_diversity()`(부하, M75/76)
  이후·`_reorder_by_execution_memory()`(특정 `required_capabilities`
  조합 성공률, M69) 이전에 `_reorder_by_benchmark()`를 추가해, M77
  Benchmark Profile(Provider 전체 누적, `_reorder_by_execution_memory()`
  보다 넓은 범위·큰 표본)로 한 번 더 tie-break한다. 정렬 키는
  `(-success_rate, average_latency_seconds)` — 성공률 내림차순이 1순위,
  완전히 같은 성공률일 때만 평균 레이턴시 오름차순으로 2차 tie-break한다
  (`failure_rate()`는 `success_rate()`의 보완값이라 별도 사용 안 함).
  안정 정렬 순서상 최종 우선순위는 cost > execution_memory(좁지만 정밀)
  > benchmark(넓지만 표본 큼) > diversity(부하, 최후순위)이며, 표본이
  `_MIN_BENCHMARK_SAMPLES`(3) 미만이거나 latency가 없으면 각각
  `_NEUTRAL_RATE`(0.5)/`math.inf`로 대체해 즉시 기존 순서(diversity
  결과)로 fallback한다 — 페널티도 우대도 주지 않는다. 새 상태를 만들지
  않고 M77이 이미 조합해 둔 값만 재사용하며, `engine_selection_policy`
...
- **EngineRuntime 확장 정책**: `LearningRuntimeAnalyzer`(M54)/
  `NegotiationCoordinator`(M84)가 반복한 옵트인 계층 확장 패턴은
  `.ai/RULES.md` §1.7(EngineRuntime Extension Policy)에 명문화되어
  있다 — `EngineRuntime`에 새 메서드를 추가하기 전에 먼저 검토해야
  하는 절차.
...
- **의존 방향**: Core Domain(`domain`/`interfaces`/`engines`)은
  ...
  에서만 조립한다(ADR-0034).
- **배포 대상(Deployment Target, 2026-08-02)**: `web/`(Dashboard/
  Automation/Production API)와 `runtime/production/`은 **로컬
  전용**을 목표로 한다 — 개발자 개인 머신에서 내부 개발·디버깅
  목적으로만 실행하며, 외부 네트워크 노출은 계획되어 있지 않다.
  `ProductionConfig.host`의 기본값이 `127.0.0.1`인 것과 CORS/인증
  미들웨어가 없는 것은 이 목표와 일치한다. 이 결정에 따라 인증
  (Authentication)/인가(Authorization)/CORS는 현재 범위에서
  구현하지 않는다. `--host`/`--port`로 값을 오버라이드할 수 있는
  것은 로컬 개발 편의(예: 컨테이너 내부에서의 바인딩 주소 조정)를
  위한 것이며 외부 노출을 의도하지 않는다. 향후 원격 서버 배포·팀
  협업 환경 등 외부 접근이 공식 요구사항으로 확정되면, 그때 별도
  Action Item으로 인증·인가·CORS를 설계·구현한다(2차 독립 재감사
  Finding 4, `.ai/audit/ACTION_PLAN_2ND_AUDIT.md` AI-05 대응).
...
그 후속 작업이 §15(Milestone 46)다 — §14.2~14.4는 폐기되지 않고
§15가 그대로 재사용한다.

## 15. Vault Information Architecture (ADR-0064, Milestone 46, 2026-07-31)

**목적**: M39~M45로 AI Workspace의 기능적 아키텍처(Recommendation/
Execution/Memory/Experience/Guardian/Observability/Explainability/
Orchestration)는 안정화됐지만, Obsidian Vault는 Milestone 진행
과정에서 점진적으로 성장해 일관된 Long-term Memory 구조라고 보기
어려웠다. 이 Milestone은 **기능 변경 없이** Vault를 "문서 저장소"
가 아니라 "AI Workspace의 Long-term Memory Layer"로 재정의한다.

**실측 기반 분석(T01)**: Vault 49개 Markdown 문서를 스크립트로 전수
분석 — Frontmatter는 100% 커버리지지만 `type` 필드는 13/49만 존재,
Tag는 대부분 1회성(재사용 0회), `15 Project Intelligence/`의 AI
생성 리포트 6개는 outgoing link가 전혀 없고, 결정적으로 `ADR
Index.md`/`Milestones Index.md`가 관련 파일을 **백틱 텍스트로만**
언급하고 `[[WikiLink]]`를 쓰지 않아 Graph/Backlink에 실제 지식
구조가 드러나지 않음을 확인했다. 반대로 `00 System/PROJECT_INDEX.md`
는 이미 사실상 MOC(Map of Content)로 기능하고 있다는 강점도
확인했다.

**Graphify/Second Brain 대비 채택·수정·기각(T02)**: Graphify
철학을 그대로 복사하지 않고 7개 항목(Knowledge Graph First/MOC/
Wiki Link First/Metadata First/Project·Label Standard/Concept/
Document Type Color)마다 근거를 들어 판단했다 — 예: "모든 것을
Node로 만든다"는 원문은 기각(코드 심볼까지 Node화하면 GitHub
원본과 이중 관리가 됨)하고 "GitHub 원문을 대표하는 얇은 Vault
Node + Wiki Link"로 수정 채택. Dataview는 Community Plugin이 현재
전혀 설치돼 있지 않음을 실측 확인해 이번에는 기각(Desktop 검증
이후 재검토 조건부).

**Node/Relationship Definition(T03)**: ADR/Milestone/Decision/
Concept/Project Intelligence는 Node로 정의하되, PR과 Runtime
(Observability)은 Node화하지 않는다(PR은 GitHub 네이티브 트랜잭션,
Runtime은 휘발성 표시일 뿐 영속 문서가 아님). 관계 9종
(implements/documents/belongs_to/generated_by/explains/references/
related_to/decides/supersedes)을 정의하되 별도 Frontmatter 관계
필드는 도입하지 않는다(최소 Metadata 원칙).

**Document Type Color Strategy는 §14.2를 폐기하지 않고 확장한다**:
기존 6-Cluster(Domain 기준) 색상은 전부 그대로 재사용하고, Document
Type이라는 새 1차 축을 추가한다 — ADR/Milestone/Decision/Concept
처럼 §14.2에 없던 유형만 Purple 계열 변주로 최소 추가한다.
`.obsidian/graph.json` 실제 적용은 **이번에 하지 않는다** — 2026-
07-30 사용자 결정으로 동결된 Pending Verification 상태(Desktop
검증 대기)를 그대로 유지한다.

**Migration Plan은 삭제 없는 증분 방식**: 기존 문서 삭제 0건, 이번
PR은 Phase 0(IA 문서 5개 신규 생성 + `PROJECT_INDEX.md`에 진입점
1줄 추가)만 실행한다. Recommendation Hub/Concept 문서 8종/기존
문서 `type` 일괄 추가/Roadmap Hub/Color 실제 적용은 전부 별도
Phase로 제안만 하고 이번 범위에 포함하지 않는다(Boy Scout Rule과
동일 정신 — 실제로 그 문서를 건드릴 이유가 생겼을 때 점진 적용).

**산출물**(전부 Vault 신규 문서, 기존 문서는 `PROJECT_INDEX.md`
1줄 추가만): `02 Architecture/Vault Information Architecture.md`
(마스터 문서), `Metadata Standard.md`, `Document Type Color
Strategy.md`, `Map of Content Guide.md`, `Vault Migration Plan.md`.

**범위 밖(Non-goals)**: 코드/기능 변경(Recommendation/Execution/
Guardian/Automation/Memory/Observability/Explainability/
Orchestration 전부 무변경), `.obsidian/graph.json` 실제 수정,
Dataview Plugin 설치, Concept 문서 8종 실제 생성(Phase 2로 제안만),
기존 문서 일괄 Metadata 추가. 새 Core Domain Interface/Adapter
없음(27종 유지, 애초에 코드 변경이 없음).

### 15.1 Knowledge Graph Migration (Milestone 47, ADR-0064 구현 단계, 2026-07-31)

**목적**: §15가 설계만 하고 의도적으로 미룬 Phase 1~3(Recommendation
Hub/Concept 문서/Metadata 백필)을 실제 Vault 전체에 적용한다 —
새로운 Architecture 결정이 아니라 ADR-0064의 **구현**이다(아래
"ADR 필요 여부" 참고). 기능 변경은 여전히 없다.

- **Phase 1 완료**: `15 Project Intelligence/Recommendation Hub.md`
  생성 — Recommendation 파이프라인 5단계(M35→M42→M43→M36→M44)가
  만드는 3개 리포트(전부 outgoing link 0건이었음)를 처음으로 서로
  연결. 리포트 파일 자체는 여전히 수정하지 않는다(자동 생성 규칙
  유지).
- **Phase 2 완료(8종 아니라 7종)**: `02 Architecture/Concepts/*.md`
  7개(Recommendation/Execution/Memory/Guardian/Observability/
  Automation/Runtime) + `Concept Index.md` 생성. §15가 원래 열거한
  8종(Adaptation/Orchestration/Explainability 포함)을 기계적으로
  만들지 않고, 이 3개는 Recommendation 파이프라인의 **단계**일 뿐
  독립된 Domain Vocabulary가 아니라는 판단으로 Concept 문서 없이
  Recommendation Hub의 흐름 표로만 표현했다 — Reuse First 정신을
  Concept 생성에도 적용.
- **Phase 3 완료(Boy Scout Rule → 전수 백필로 승격)**: §15의
  Migration Plan은 원래 Boy Scout Rule(점진 적용)을 제안했으나,
  이번 Milestone은 사용자가 "Metadata Backfill 완료"를 DoD로
  명시해 스크립트 기반 **전수 백필**로 승격했다 — 기존 36개 문서
  전부에 `type`을 추가해 Vault 54개 문서 100% 커버리지를 달성했다.
- **Hub 3종 신규 추가(§15 원안에 없던 확장)**: Architecture Hub/
  Runtime Hub/Decision Hub/Knowledge Hub — 기존 Architecture
  Overview/ADR Index/Decisions Index/Milestones Index를 대체하지
  않고 그 위에 얇은 진입점만 추가한다(내용 복제 없음).
- **Lessons 구조 신규 추가(§15 T03이 YAGNI로 보류했던 것)**: `16
  Lessons/Lessons.md` — 실제 Lesson 항목은 아직 없다(추측/허위
  데이터 생성 금지 원칙 유지). `13 Daily`/`14 Tasks`와 동일한 패턴
  (구조는 준비, 항목은 필요할 때). `VAULT_CONTENT_DIRECTORIES`
  (`vault/mapping.py`)에 17번째 디렉터리로 등록 — Vault 검증
  스캔 범위에 포함시키기 위한 최소 설정 변경(Recommendation 등
  8개 보호 기능과 무관, Integration Layer 상수 목록).
- **Phase 5(Color Migration)는 계속 Pending**: `.obsidian/
  graph.json`은 여전히 수정하지 않는다 — 2026-07-30 동결 결정
  그대로. Metadata `type` 필드가 이제 100% 채워져 있어 검증이
  풀리는 즉시 적용 가능한 상태로만 대비했다.
- **ADR 필요 여부**: 새 ADR을 작성하지 않는다. 이번 작업은 새로운
  아키텍처 결정이 아니라 ADR-0064가 이미 설계·승인한 Migration
  Plan을 그대로 실행한 것이다 — Domain Vocabulary/Node 정의/
  Color 원칙/Backward Compatibility 원칙 중 무엇도 바꾸지 않았다.
  유일한 코드 변경(`VAULT_CONTENT_DIRECTORIES`에 `16 Lessons`
  추가)도 ADR-0064 T03의 Lesson Role 정의를 그대로 구현한 것일 뿐
  새 정책이 아니다.
- **Migration 검증**: `type` 커버리지 13/49(27%)→54/54(100%), Orphan
  문서 4→3, Recommendation 파이프라인 리포트의 inbound 링크
  0건→1~4건씩, `find_broken_backlinks()` 신규 문서 전수 통과.
  상세는 [[Vault Migration Plan]]의 "Migration 검증" 절.
- **범위 밖(Non-goals)**: 코드/기능 변경 전부 무변경,
  `.obsidian/graph.json` 실제 수정, Roadmap Hub(§15 Phase 4, 별도
  승인 필요), Dataview Plugin 설치, 허구 Lesson 데이터 생성.
