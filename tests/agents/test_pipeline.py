...
def test_mission_planned_triggers_full_event_chain() -> None:
    """T2-06 DoD: MissionPlanned→CodeCompleted→ReviewCompleted→
    DocumentationCompleted Event 체인이 Agent 간 협업으로 자동 진행된다.
    InMemoryEventBus는 핸들러 내부에서 재귀적으로 publish()가 호출되면
    수신 순서가 뒤집히므로(가장 안쪽에서 발행된 이벤트를 먼저 관측), 순서가
    아니라 수신된 이벤트 타입의 집합과 최종 Task 상태로 체인 완주를
    검증한다."""
