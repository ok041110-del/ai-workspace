...
def test_build_command_uses_prompt_flag_with_json_output_and_yolo() -> None:
...
def test_build_command_appends_model_when_given() -> None:
    provider = GeminiCliProvider(model="gemini-3-pro")

    command = provider.build_command("s1", make_task())

    assert command[-2:] == ["--model", "gemini-3-pro"]
...
def test_capabilities_and_supports_parallel() -> None:
    ...
    assert "gemini" in provider.capabilities()
